Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsSolidRocketEngine_NET.clsSolidRocketEngine")> Public Class clsSolidRocketEngine
	...
	Private mvarBurnTime As Single
	...
	Public Property BurnTime() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.BurnTime
			BurnTime = mvarBurnTime
		End Get
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Pod) Or (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Wing) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Solid Rockets must be placed in Body, Wing, Pod, equipment Pod or Leg")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(SolidRocketMatrix)
			If SolidRocketMatrix(i).ID = mvarDatatype Then
				If SolidRocketMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		mvarWeight = SolidRocketMatrix(mvarMatrixPos).Weight * mvarDesiredThrust * mvarBurnTime
		mvarCost = mvarWeight * SolidRocketMatrix(mvarMatrixPos).Cost
		mvarVolume = mvarWeight / SolidRocketMatrix(mvarMatrixPos).Volume
