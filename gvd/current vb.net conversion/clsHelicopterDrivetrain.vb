Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsHelicopterDrivetrain_NET.clsHelicopterDrivetrain")> Public Class clsHelicopterDrivetrain
	...
	Private mvarTiltRotor As Boolean
	...
	Public Property TiltRotor() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.TiltRotor
			TiltRotor = mvarTiltRotor
		End Get
		Set(ByVal Value As Boolean)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.TiltRotor = 5
			mvarTiltRotor = Value
		End Set
	End Property
	...
	Public Function LocationCheck() As Boolean
		...
		Dim rotorarray() As String
		Dim rotordrivetrainarray() As String
		...
		rotorarray = VB6.CopyArray(Veh.KeyManager.GetCurrentRotorKeys)
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.KeyManager. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		rotordrivetrainarray = VB6.CopyArray(Veh.KeyManager.GetCurrentRotorDrivetrainKeys)
		
		If rotorarray(1) = "" Then
			modHelper.InfoPrint(1, "Install more Rotors first.  You cannot have more rotor drivetrains than you have rotor subassemblies.")
			TempCheck = False
			LocationCheck = TempCheck
			Exit Function
		ElseIf rotordrivetrainarray(1) = "" Then 
		ElseIf UBound(rotorarray) <= UBound(rotordrivetrainarray) Then 
			modHelper.InfoPrint(1, "Install more Rotors first.  You cannot have more rotor drivetrains than you have rotor subassemblies.")
			TempCheck = False
			LocationCheck = TempCheck
			Exit Function
		...
		'determine if the user is adding the drivetrain to the correct part of the vehicle
		...
		Select Case mvarDatatype
			Case MMRRotorDrivetrain
				If (InstallPoint = Pod) Or (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Turret) Or (InstallPoint = Superstructure) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "MMR Rotor Drivetrain must be placed in Body, Turret, Superstructure or Pod")
					TempCheck = False
				End If
			Case Else
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Turret) Or (InstallPoint = Superstructure) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "TTR and CAR Rotor Drivetrains must be placed in Body, Turret or Superstructure")
					TempCheck = False
				End If
		End Select
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case TTRRotorDrivetrain
				
			Case CARRotorDrivetrain
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(HelicopterMatrix)
			If HelicopterMatrix(i).ID = mvarDatatype Then
				If HelicopterMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'If MotivePower = 0 Then Exit Sub
		...
		If mvarMotivePower >= 5 Then
			TempWeight1 = HelicopterMatrix(mvarMatrixPos).Weight2
			TempWeight2 = HelicopterMatrix(mvarMatrixPos).Weight3
		Else
			TempWeight1 = HelicopterMatrix(mvarMatrixPos).Weight1
			TempWeight2 = 0
		End If
		
		'add mods for MMR rotor tilt rotor
		If mvarTiltRotor Then
			TiltRotorMod = 1.5
		Else
			TiltRotorMod = 1
		End If
		...
		'get base stats
		mvarWeight = TiltRotorMod * ((mvarMotivePower * TempWeight1) + TempWeight2)
		mvarCost = mvarWeight * HelicopterMatrix(mvarMatrixPos).Cost
		mvarVolume = mvarWeight / HelicopterMatrix(mvarMatrixPos).Volume
		...
		mvarLift = System.Math.Round(mvarMotivePower * HelicopterMatrix(mvarMatrixPos).Lift, 2)
		mvarMotiveThrust = System.Math.Round(mvarMotivePower * HelicopterMatrix(mvarMatrixPos).MotiveThrust, 2)
