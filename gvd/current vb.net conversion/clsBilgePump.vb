Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsBilgePump_NET.clsBilgePump")> Public Class clsBilgePump
	...
	Public Function LocationCheck() As Boolean
		...
		If InstallPoint = Body Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Bilge Pumps must be placed in the hull.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(BilgePumpMatrix)
			If BilgePumpMatrix(i).ID = mvarDatatype Then
				If BilgePumpMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'//determine how many we actually have to pay for
		...
		If Veh.Stats.TotalVolume <= 100 Then
			QuantityMod = mvarQuantity
		Else
			'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Stats. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			QuantityMod = Veh.Stats.TotalVolume / 1000
			If QuantityMod > System.Math.Round(QuantityMod, 0) Then
				QuantityMod = System.Math.Round(QuantityMod, 0) + 1
			End If
			QuantityMod = mvarQuantity - QuantityMod
			If QuantityMod <= 0 Then QuantityMod = 0
		End If
		
		'get base stats
		mvarWeight = BilgePumpMatrix(mvarMatrixPos).Weight * QuantityMod
		mvarCost = BilgePumpMatrix(mvarMatrixPos).Cost * QuantityMod
		mvarVolume = BilgePumpMatrix(mvarMatrixPos).Volume * QuantityMod
		...
		mvarPowerReqt = System.Math.Round(mvarQuantity * BilgePumpMatrix(mvarMatrixPos).Power, 2)
