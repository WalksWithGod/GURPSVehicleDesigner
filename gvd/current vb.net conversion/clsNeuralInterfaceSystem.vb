Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsNeuralInterfaceSystem_NET.clsNeuralInterfaceSystem")> Public Class clsNeuralInterfaceSystem
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Neural Interface Systems must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case SurgicalInterface
				
			Case InterfaceWeb
				
			Case AutoInterfaceWeb
				
			Case SocketInterface
				
			Case NeuralInductionField
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(NeuralInterfaceMatrix)
			If NeuralInterfaceMatrix(i).ID = mvarDatatype Then
				If NeuralInterfaceMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		mvarWeight = System.Math.Round(mvarQuantity * NeuralInterfaceMatrix(mvarMatrixPos).Weight, 2)
		mvarCost = System.Math.Round(mvarQuantity * NeuralInterfaceMatrix(mvarMatrixPos).Cost, 2)
		mvarVolume = System.Math.Round(mvarQuantity * NeuralInterfaceMatrix(mvarMatrixPos).Volume, 2)
		mvarPowerReqt = NeuralInterfaceMatrix(mvarMatrixPos).Power * mvarQuantity
		...
		If Volume <> 0 Then
			...
			mvarHitPoints = modPerformance.Maximum(1, System.Math.Round(mvarSurfaceArea, 0))
		End If
		
		'produce the print output
		If mvarQuantity <> 0 Then
			sPrint1 = ", " & VB6.Format(mvarQuantity) & " users"
		End If
		
		mvarPrintOutput = " TL" & mvarTL & " " & mvarCustomDescription & sPrint1 & " (" & mvarLocation & ", HP " & mvarHitPoints & ", " & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
