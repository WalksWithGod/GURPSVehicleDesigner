Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsLabandWorkshop_NET.clsLabandWorkshop")> Public Class clsLabandWorkshop
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Labs and Workshops must be placed in Body, Superstructure, Turret, Popturret or Pod.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			...
			Case MechanicWorkshop
				
			Case ElectronicsWorkshop
				
			Case EngineeringWorkshop
				
			Case ArmouryWorkshop
				
			Case CompleteMiniWorkshop
				
			Case CompleteWorkshop
				
			Case MiniMechanicWorkshop
				
			Case MiniElectronicsWorkshop
				
			Case MiniEngineeringWorkshop
				
			Case MiniArmouryWorkshop
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(WorkshopMatrix)
			If WorkshopMatrix(i).ID = mvarDatatype Then
				If WorkshopMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = WorkshopMatrix(mvarMatrixPos).Weight
		mvarCost = WorkshopMatrix(mvarMatrixPos).Cost
		mvarVolume = WorkshopMatrix(mvarMatrixPos).Volume
		...
		'check for physics lab power modification
		If mvarSkill = "Physics" Then
			mvarPowerReqt = System.Math.Round(mvarQuantity * WorkshopMatrix(mvarMatrixPos).Power * WorkshopMatrix(mvarMatrixPos).PhysicsMod, 2)
		Else
			mvarPowerReqt = System.Math.Round(mvarQuantity * WorkshopMatrix(mvarMatrixPos).Power, 2)
		End If
		'print output
		If mvarRuggedized Then
			sPrint1 = "ruggedized "
