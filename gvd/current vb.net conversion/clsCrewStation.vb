Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsCrewStation_NET.clsCrewStation")> Public Class clsCrewStation
	
	Private mvarWeight As Double
	Private mvarVolume As Double
	Private mvarCost As Double
	Private mvarStationFunction As String
	...
	Private mvarExtraSpace As Single
	Private mvarGSeat As Boolean
	Private mvarBridgeAccessSpace As Boolean
	...
	Public Property BridgeAccessSpace() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.BridgeAccessSpace
			BridgeAccessSpace = mvarBridgeAccessSpace
		End Get
		Set(ByVal Value As Boolean)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.BridgeAccessSpace = 5
			mvarBridgeAccessSpace = Value
		End Set
	End Property
	...
	Public Property StationFunction() As String
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.StationFunction
			StationFunction = mvarStationFunction
		End Get
		Set(ByVal Value As String)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.StationFunction = 5
			mvarStationFunction = Value
		End Set
	End Property
	...
	Public Property ExtraSpace() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.ExtraSpace
			ExtraSpace = mvarExtraSpace
		End Get
		Set(ByVal Value As Single)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.ExtraSpace = 5
			mvarExtraSpace = Value
		End Set
	End Property
	...
	Public Function LocationCheck() As Boolean
		...
		Select Case mvarDatatype
			...
			Case CrampedCrewStation, NormalCrewStation, RoomyCrewStation
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = OpenMount) Or (InstallPoint = Pod) Or (InstallPoint = Arm) Or (InstallPoint = Leg) Or (InstallPoint = Wing) Or (InstallPoint = equipmentPod) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Cramped, Normal, and Roomy Crew Stations must be placed in Body, Superstructure, Turret, Popturret, Open Mount, Pod, equipment Pod, Wing, Arm, or Leg.")
					TempCheck = False
				End If
			Case CycleCrewStation, HarnessCrewStation
				If InstallPoint = Body Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Cycle and Harness Crew Stations must be placed in hull.")
					TempCheck = False
				End If
	...
	Private Sub Class_Initialize_Renamed()
		...
		mvarGSeat = False
		mvarExposed = False
		mvarStationFunction = "Pilot controls everything from"
		mvarQuantity = 1
	End Sub
	Public Sub New()
		MyBase.New()
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case CrampedCrewStation
				
			Case NormalCrewStation
				
			Case RoomyCrewStation
				
			Case CycleCrewStation
				
			Case HarnessCrewStation
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(CrewStationMatrix)
			If CrewStationMatrix(i).ID = mvarDatatype Then
				If CrewStationMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		If mvarBridgeAccessSpace Then
			VolumeMod = VolumeMod * 3
		End If
		
		mvarWeight = System.Math.Round(CrewStationMatrix(mvarMatrixPos).Weight, 2)
		mvarCost = System.Math.Round(CrewStationMatrix(mvarMatrixPos).Cost + CostMod, 2)
		mvarVolume = System.Math.Round(VolumeMod * CrewStationMatrix(mvarMatrixPos).Volume, 2)
		...
		'todo: make a "bridge" container object and when crew stations are added to it, bridge access space gets
		' added by the bridge based on number of children it has.
		If mvarBridgeAccessSpace Then
			sPrint2 = sPrint2 & ", with " & VB6.Format(mvarVolume / 3, p_sFormat) & " cf bridge access space"
		End If
		
		mvarPrintOutput = mvarStationFunction & NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & sPrintPlural & sPrint2 & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
