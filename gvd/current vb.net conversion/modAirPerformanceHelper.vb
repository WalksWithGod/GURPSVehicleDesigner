Option Strict Off
Option Explicit On
Module modAirPerformanceHelper
	
	Function CalcAMotiveThrust(ByRef KeyChain As Object, ByRef sngReservedVectoredThrust As Single, ByVal bTreatTiltRotorsAsPropellers As Boolean, ByVal sngTiltRotorForwardThrust As Single, ByVal sngPercentThrust As Single, ByVal bAfterBurnersOn As Boolean, ByVal lngPerformanceType As Integer, ByVal sngTotalRamJetThrust As Single, ByVal sngTotalRamJetThrustAB As Single, ByVal sngTotalTurboRamJetThrust As Single, ByVal sngTotalTurboramJetThrustAB As Single) As Single
		'JAW 2000.05.28
...
ErrHandler: 
			...
			TempThrust = 0
			Select Case dType
				Case CStr(CARRotorDrivetrain), CStr(MMRRotorDrivetrain), CStr(TTRRotorDrivetrain)
					...
					Else
						...
						TempThrust = Veh.Components(sKey).MotiveThrust * sngPercentThrust
					End If
				Case CStr(OrnithopterDrivetrain), CStr(AerialPropeller), CStr(RopeHarness), CStr(YokeandPoleHarness), CStr(ShaftandCollarHarness), CStr(WhiffletreeHarness), CStr(AerialSail), CStr(AerialSailForeAftRig)
				...
				Case CStr(DuctedFan) '//dont add lift engine OR hoverfan lift
					'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					If Veh.Components(sKey).LiftEngine Then
				...
				Case CStr(LiquidFuelRocket), CStr(MOXRocket), CStr(IonDrive), CStr(FissionRocket), CStr(FusionRocket), CStr(OptimizedFusion), CStr(AntimatterThermal), CStr(AntimatterPion), CStr(StandardThruster), CStr(SuperThruster), CStr(MegaThruster), CStr(SolidRocketEngine), CStr(OrionEngine)
	...
	Function GetUseableVectoredThrust(ByVal sngThrustNeeded As Single, ByRef KeyChain As Object, ByVal sngPercentThrust As Single) As Single
		...
		For i = 1 To UBound(KeyChain)
			...
			dType = Veh.Components(sKey).Datatype
			Select Case dType
				Case DuctedFan, LiquidFuelRocket, MOXRocket, IonDrive, FissionRocket, FusionRocket, OptimizedFusion, AntimatterThermal, AntimatterPion, StandardThruster, SuperThruster, MegaThruster, SolidRocketEngine, TurboRamjet, Ramjet, Turbojet, Turbofan, Hyperfan, FusionAirRam
					'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	...
	Function CalcADrag(ByVal bPopTurretsExtended As Boolean, ByVal bWheelsSkidsExtended As Boolean, ByVal bResponsive As Boolean) As Single
		
		Dim element As Object
		Dim dType As Short
		Dim Sa As Single 'surface area of entire vehicle
		Dim R As Single 'surface area of anything retractable
		Dim Sl As Integer 'streamlining modifier
		Dim D As Integer
		...
		For	Each element In Veh.Components
			...
			ElseIf TypeOf element Is clsWheel Then 
				'UPGRADE_WARNING: Couldn't resolve default property of object element.RetractLocation. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		...
		For	Each element In Veh.Components
			...
			If (TypeOf element Is clsAccommodation) Or (TypeOf element Is clsCrewStation) Then
				...
				If dType = CycleSeat Then
					...
					'UPGRADE_WARNING: Couldn't resolve default property of object element.Exposed. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	...
	Function CalcAAcceleration(ByVal Thrust As Single, ByVal Weight As Double) As Single
		...
		CalcAAcceleration = System.Math.Round(TempAcceleration, 0)
	End Function
	...
	Function CalcAManeuverability(ByVal sngStallSpeed As Single, ByVal bResponsive As Boolean, ByVal Weight As Double) As Single
		...
		Dim Rhp As Integer
		Dim Lwt As Single
		Dim RotorTL As Short
		Dim WingTL As Short
		Dim TempTL As Short 'Rotor hit points 'wing hit points
		...
		TempM = modPerformance.Maximum(TempM, TempM2)
	...
	Function CalcAStability() As Single
		Dim TempSR As Object
		
		'first get the initial SR based on vehicle volume
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Stats. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		Select Case Veh.Stats.TotalVolume
			Case Is <= 99
				'UPGRADE_WARNING: Couldn't resolve default property of object TempSR. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				TempSR = 2
			Case Is <= 999
				'UPGRADE_WARNING: Couldn't resolve default property of object TempSR. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				TempSR = 3
	...
	Function CalcATopSpeed(ByVal Drag As Single, ByVal Thrust As Single, ByVal bAfterBurnersOn As Boolean, ByVal sngTotalRamJetThrust As Single, ByVal sngTotalTurboRamJetThrust As Single, ByVal sngTotalRamJetThrustAB As Single, ByVal sngTotalTurboramJetThrustAB As Single) As Single
		...
		TempSpeed = System.Math.Sqrt(7500 * (Thrust / Drag)) 'do the main calculation
		...
		If TempSpeed >= 375 Then
			...
			End If
			TempSpeed = System.Math.Sqrt(7500 * (Thrust / Drag)) 'do the updated calculation
		End If
	...
	Function CalcAMaxSpeed(ByVal TempSpeed As Single, ByRef KeyChain As Object, ByVal bTreatTiltRotorsAsPropellers As Boolean, ByRef sDesignCheck As String) As Single
		Dim Sl As String
		...
		If GetLowestDR < 5 Then
			...
			TempSpeed = modPerformance.Minimum(TempSpeed, 600)
			If bSpeed <> TempSpeed Then
		...
		ElseIf GetLowestDR < 20 Then 
			'UPGRADE_WARNING: Couldn't resolve default property of object Minimum(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			TempSpeed = modPerformance.Minimum(TempSpeed, 2000)
			If bSpeed <> TempSpeed Then
		...
		ElseIf Sl = "very good" Then 
			...
			If Not Veh.Components(BODY_KEY).LiftingBody Then
				'UPGRADE_WARNING: Couldn't resolve default property of object Minimum(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				TempSpeed = modPerformance.Minimum(TempSpeed, 740)
				If bSpeed <> TempSpeed Then
		...
		If VehicleHasWings Then
			If VehicleHasFlarecraftWings Then
				'UPGRADE_WARNING: Couldn't resolve default property of object Minimum(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				TempSpeed = modPerformance.Minimum(TempSpeed, 400)
				If bSpeed <> TempSpeed Then
		...
		Else
			If VehicleHasRotors Then
				If GetLowestTL("clsRotor") <= 6 Then
					'UPGRADE_WARNING: Couldn't resolve default property of object Minimum(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					TempSpeed = modPerformance.Minimum(TempSpeed, 150)
					If bSpeed <> TempSpeed Then
		...
		If (PropulsionSystemExistsOnKeychain(AerialSail, KeyChain)) Then
			'UPGRADE_WARNING: Couldn't resolve default property of object Minimum(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			TempSpeed = modPerformance.Minimum(TempSpeed, 100)
			If bSpeed <> TempSpeed Then
	...
	Function PropulsionSystemExistsOnKeychain(ByVal Datatype As Short, ByRef KeyChain As Object) As Boolean
		Dim i As Integer
		Dim sKey As String
		Dim dType As Short
