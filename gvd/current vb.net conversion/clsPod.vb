Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsPod_NET.clsPod")> Public Class clsPod
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = Wing) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Pods must be placed on Body or Wing.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		mvarZZInit = 1
		'get the accessspace
		'NOTE: pods require 0 access space
		...
		mvarVolume = System.Math.Round(CalcCombinedVolume(mvarKey) + (mvarEmptySpace * mvarQuantity) + (mvarAccessSpace * mvarQuantity), 2)
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarPrintOutput = CStr(CDbl(NumericToString(mvarQuantity) & " " & mvarCustomDescription & sPrintPlural1 & " " & " (on " & mvarOrientation & " of ") + Veh.Components(mvarParent).CustomDescription + CDbl(")."))
