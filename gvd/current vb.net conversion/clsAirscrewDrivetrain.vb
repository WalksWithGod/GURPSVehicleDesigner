Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsAirscrewDrivetrain_NET.clsAirscrewDrivetrain")> Public Class clsAirscrewDrivetrain
		...
		Select Case mvarDatatype
			Case AerialPropeller
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Aerial Propeller must be placed in Body or Pod")
					TempCheck = False
				End If
		End Select
	...
	Public Sub StatsUpdate()
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & VB6.Format(mvarMotivePower, p_sFormat) & " kW " & sPrint1 & mvarCustomDescription & sPrintPlural & sPrintPlural2 & VB6.Format(mvarMotiveThrust, p_sFormat) & " lbs. thrust" & " (" & mvarLocation & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., $" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
