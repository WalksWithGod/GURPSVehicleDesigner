Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsEnergyBank_NET.clsEnergyBank")> Public Class clsEnergyBank
	...
	Public Property PoweredRewinder() As String
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.Recharging
			PoweredRewinder = CStr(mvarPoweredRewinder)
		End Get
		Set(ByVal Value As String)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.poweredrewind = 5
			mvarPoweredRewinder = CBool(Value)
			If mvarZZInit = 0 Then Exit Property
			
			If mvarPoweredRewinder = False Then MotorST = 0
	...
	Public Property MotorST() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Energy Banks must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Wing, Arm, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		If (mvarDatatype = ClockWork) And (mvarPoweredRewinder) Then
			mvarMotorST = System.Math.Round(EnergyBankMatrix(mvarMatrixPos).EffectiveST * mvarDesiredOutput, 0)
		...
		If mvarPoweredRewinder Then
			sPrint2 = ", with ST " & VB6.Format(mvarMotorST) & " powered rewinder"
		End If
		
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & VB6.Format(mvarDesiredOutput, p_sFormat) & " kWs " & sPrint1 & mvarCustomDescription & sPrintPlural & sPrint2 & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
