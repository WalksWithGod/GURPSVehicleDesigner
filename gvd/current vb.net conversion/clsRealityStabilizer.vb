Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsRealityStabilizer_NET.clsRealityStabilizer")> Public Class clsRealityStabilizer
	...
	Public Property PowerSystemKeyChain() As Object
		Get
			Dim mvarPowerSystemKeyChain As Object
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.PowerSystemKeyChain
		...
		Set(ByVal Value As Object)
			Dim mvarPowerSystemKeyChain As Object
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.PowerSystemKeyChain = 5
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Or (InstallPoint = Leg) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Reality Stabilizers must be placed in Body, Superstructure, Turret, Popturret, Pod, equipment Pod, Wing, Leg, Arm or Module.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Dim LargeRealityStabilizer As Object
		'UPGRADE_WARNING: Couldn't resolve default property of object LargeRealityStabilizer. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		Select Case mvarDatatype
			Case SmallRealityStabilizer
				
			Case MediumRealityStabilizer
				
			Case LargeRealityStabilizer
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(RealityStabilizerMatrix)
			If RealityStabilizerMatrix(i).ID = mvarDatatype Then
				If RealityStabilizerMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		mvarWeight = RealityStabilizerMatrix(mvarMatrixPos).Weight
		mvarCost = RealityStabilizerMatrix(mvarMatrixPos).Cost
		mvarVolume = RealityStabilizerMatrix(mvarMatrixPos).Volume
		...
		mvarHitPoints = CalcComponentHitpoints(RugHitMod * SurfaceArea)
		mvarPowerReqt = RealityStabilizerMatrix(mvarMatrixPos).Power
