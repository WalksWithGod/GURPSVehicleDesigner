Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsDecoyReload_NET.clsDecoyReload")> Public Class clsDecoyReload
	...
	Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Decoy Reloads must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, or Leg.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		...
		If mvarDatatype = DecoyChaff Then
			
		ElseIf mvarDatatype = DecoySmoke Then 
			
		ElseIf mvarDatatype = DecoyFlares Then 
			
		ElseIf mvarDatatype = DecoySonarDecoy Then 
			
		ElseIf mvarDatatype = DecoyHotSmoke Then 
			
		ElseIf mvarDatatype = DecoyPrism Then 
			
		ElseIf mvarDatatype = DecoyBlackOutGas Then 
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(DecoyReloadMatrix)
			If DecoyReloadMatrix(i).ID = mvarDatatype Then
				If DecoyReloadMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		mvarWeight = System.Math.Round(mvarQuantity * DecoyReloadMatrix(mvarMatrixPos).Weight, 2)
		mvarCost = System.Math.Round(mvarQuantity * DecoyReloadMatrix(mvarMatrixPos).Cost, 2)
		mvarVolume = System.Math.Round(mvarQuantity * DecoyReloadMatrix(mvarMatrixPos).Volume, 2)
		...
		Else
			...
			sPrintPlural3 = ""
		End If
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & mvarCustomDescription & sPrintPlural & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural2 & ", " & sPrintPlural3 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
