Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsSimpleCustom_NET.clsSimpleCustom")> Public Class clsSimpleCustom
	...
	Private mvarUserWeight As Single
	Private mvarUserVolume As Single
	Private mvarUserCost As Single
	...
	Public Property UserCost() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.UserCost
			UserCost = mvarUserCost
		End Get
	...
	Public Property UserVolume() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.UserVolume
			UserVolume = mvarUserVolume
		End Get
		Set(ByVal Value As Single)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.UserVolume = 5
			mvarUserVolume = Value
		End Set
	End Property
	...
	Public Property UserWeight() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.UserWeight
			UserWeight = mvarUserWeight
		End Get
		Set(ByVal Value As Single)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.UserWeight = 5
			mvarUserWeight = Value
		End Set
	End Property
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "This custom component can only be placed in a subassembly or Group.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		mvarCost = mvarUserCost
		mvarWeight = mvarUserWeight * QRugMod
		mvarVolume = mvarUserVolume * QRugMod
		...
		If mvarRuggedized Then sPrint1 = "ruggedized"
		
		mvarPrintOutput = "TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & " (" & mvarLocation & ", HP " & mvarHitPoints & ", " & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
