Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsSolarCellArray_NET.clsSolarCellArray")> Public Class clsSolarCellArray
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = SolarPanel) Or (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Wing) Or (InstallPoint = Gasbag) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Solar Cell Arrays must be placed on Solar Panels, Body, Superstructure, Turret, Popturret, Wing, or Gasbags.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		Dim ParentsArea As Single
		Dim TempPowerConsumption As Single
		Dim i As Integer
		
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		If TypeOf Veh.Components(mvarLogicalParent) Is clsGroup Then
			SetLogicalParent()
		End If
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		ParentsArea = Veh.Components(mvarLogicalParent).SurfaceArea
		mvarSurfaceArea = System.Math.Round(ParentsArea * mvarPercentCovered / 100, 2)
		...
		mvarWeight = System.Math.Round(WeightMod * mvarSurfaceArea, 2)
		mvarCost = System.Math.Round(CostMod * mvarSurfaceArea, 2)
		mvarOutput = System.Math.Round(PowerMod * mvarSurfaceArea, 2)
		'the endurance for solar panel is unlimited?
		'mvarendurance
		
		
		'produce the print output
		
		mvarPrintOutput = "TL" & mvarTL & " " & VB6.Format(mvarOutput, p_sFormat) & " kW " & mvarCustomDescription & " (" & mvarLocation & ", " & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
