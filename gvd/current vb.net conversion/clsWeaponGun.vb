Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsWeaponGun_NET.clsWeaponGun")> Public Class clsWeaponGun
	...
	Private mvarDXPlusSkill As Integer
	...
	Public Property Carriage() As Boolean
		Get
	...
	Public Property BoxMagazine() As Boolean
		Get
	...
	Public Property DXPlusSkill() As Integer
		Get
	...
	Public Property sRoF() As String
		Get
	...
	Public Property dRoF() As Single
		Get
	...
	Public Property Electric() As Boolean
		Get
		...
		Set(ByVal Value As Integer)
			...
			If (mvarDatatype = ElectricGatling) Or (mvarDatatype = HeavyAutomatic) Or (mvarDatatype = lightAutomatic) Then
				...
				For I = 1 To UBound(rofarray)
					If CDbl(rofarray(I)) = mvardRof Then
						bValidRoF = True
						Exit For
					End If
				Next 
				...
				If bValidRoF = False Then
					modHelper.InfoPrint(1, "RoF not supported at this Bore Size.  Resetting to RoF  to 3.")
					mvardRof = 3
				End If
			End If
	...
	Public Property AdvancedOption() As String
		Get
	...
	Public Property PowerOption() As String
		Get
	...
	Public Property Recoiless() As Boolean
		Get
	...
	Public Property BoreSize() As Single
		...
		Set(ByVal Value As Single)
			...
			If bValidAmmoType = False Then
				modHelper.InfoPrint(1, "Ammunition type not supported at this Bore Size.  Resetting to Solid ammunition type.")
	...
	Public Sub StatsUpdate()
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarBoreSize & "mm " & mvarCustomDescription & sPrintPlural & sPrint2 & " (" & mvarLocation & sPrintDirection & ", HP " & mvarHitPoints & sPrintPlural2 & ", " & sPrintPlural3 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
	...
	Sub GetTypeDamages()
		...
		Select Case AmmoMatrix(I).Name
			Case "Solid", "Chainshot", "AP", "APC", "API", "APCR", "HP", "Plastic", "Baton", "Needle", "APDS", "APS", "APDU", "APDSDU", "APFSDS", "APFSDSDU", "Superwire", "APSHD", "APDSHD", "APFSDSHD", "Shotshell"
			...
			Case "Canister"
				'UPGRADE_WARNING: Couldn't resolve default property of object ConvertDamage(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				mvarDamage1 = ConvertDamage(System.Math.Sqrt(mvarKEDamage))
	...
	Private Function GetDamage() As Double
		...
		Dim G As Short 'gun technology modifier
	...
	Private Function GetHalfDamage() As Double 'in yards
		...
		Dim s As Short 'bore SIZE modifier
		Dim G As Short 'gun technology modifier
		Dim l As Single 'barrel length modifier
		...
		TempHalfDamage = System.Math.Sqrt(mvarBoreSize) * s * G * l * P * T
		
		' Do the final rounding
		If TempHalfDamage <= 100 Then
			' round to nearest yard
			TempHalfDamage = System.Math.Round(TempHalfDamage, 0)
		ElseIf TempHalfDamage <= 1000 Then  ' round to nearest 10 yards
			TempHalfDamage = System.Math.Round(TempHalfDamage / 10, 0) * 10
		ElseIf TempHalfDamage <= 10000 Then  ' round to nearest 100 yards
			TempHalfDamage = System.Math.Round(TempHalfDamage / 100, 0) * 100
		ElseIf TempHalfDamage > 10000 Then  ' round to nearest 1000 yards
			TempHalfDamage = System.Math.Round(TempHalfDamage / 1000, 0) * 1000
		End If
	...
	Private Function GetMaxRange() As Double 'in yards
		Dim R As Short ' holds range modifier
		...
		TempMax = (System.Math.Sqrt(mvarhalfDamage) * R) + mvarhalfDamage
		
		' Do the final rounding
		If TempMax <= 100 Then
			' round to nearest yard
			TempMax = System.Math.Round(TempMax, 0)
		ElseIf TempMax <= 1000 Then  ' round to nearest 10 yards
			TempMax = System.Math.Round(TempMax / 10, 0) * 10
		ElseIf TempMax <= 10000 Then  ' round to nearest 100 yards
			TempMax = System.Math.Round(TempMax / 100, 0) * 100
		ElseIf TempMax > 10000 Then  ' round to nearest 1000 yards
			TempMax = System.Math.Round(TempMax / 1000, 0) * 1000
		End If
	...
	Private Function GetRoF() As String
		...
		Select Case mvarDatatype
			Case MuzzleLoader
				If mvarCylinders > 1 Then
					Numerator = VB6.Format(mvarCylinders) & ":"
				Else
					Numerator = "1/"
				...
				Else
					...
					If mvarPowerOption <> "normal" Then
						Denominator = System.Math.Round(Denominator, 0)
					Else
						Denominator = System.Math.Round(Denominator / 2, 0)
					End If
				End If
			...
			Case SlowAutoloader
				If mvarBoreSize <= 30 Then
					Denominator = 1
				Else
					Denominator = System.Math.Round(mvarBoreSize / 20, 0)
					Numerator = "1/"
				End If
			...
			Case FastAutoloader
				...
				ElseIf mvarBoreSize <= 60 Then 
					Denominator = 1
				Else
					Denominator = System.Math.Round(mvarBoreSize / 40, 0)
					Numerator = "1/"
				End If
				
			Case MechanicalGatling
				If mvarBoreSize <= 60 Then
					Denominator = System.Math.Round(mvarDXPlusSkill / 2, 0)
				Else
					Denominator = System.Math.Round(mvarDXPlusSkill / 3, 0)
				End If
		End Select
		
		'save the String ROF
		If Numerator = "" Then
			mvarsRoF = VB6.Format(Denominator)
			mvardRof = Denominator
		Else
			mvarsRoF = Numerator & VB6.Format(Denominator)
			mvardRof = 1 / Denominator
		End If
	End Function
	
	Public Function FillRoFList() As String()
		Dim iTempRof As Integer
		Dim I As Integer
		...
		Select Case mvarDatatype
			Case lightAutomatic
				...
				Else
					If mvarBoreSize > 160 Then
						iTempRof = 1
					Else
						iTempRof = System.Math.Round(160 / mvarBoreSize, 0)
					End If
				End If
			...
			Case HeavyAutomatic
				...
				Else
					If mvarBoreSize > 400 Then
						iTempRof = 1
					Else
						iTempRof = System.Math.Round(400 / mvarBoreSize, 0)
					End If
				End If
				If iTempRof < 3 Then iTempRof = 3
				ReDim rofarray(iTempRof - 2)
			...
			Case ElectricGatling
				If mvarBoreSize <= 20 Then
					iTempRof = System.Math.Round(16.67 * mvarCylinders, 0)
				Else
					If mvarBoreSize > 334 Then
						iTempRof = mvarCylinders
					Else
						iTempRof = System.Math.Round((334 / mvarBoreSize) * mvarCylinders, 0)
					End If
				End If
		...
		FillRoFList = VB6.CopyArray(rofarray)
	End Function
	...
	Private Function GetWeightPerShot() As Double
		...
		Dim l As Single 'barrel length modifier
		Dim Divisor As Integer 'tl5- smoothbore modifier
		...
		GetWeightPerShot = System.Math.Round(sngRet, 5)
	End Function
	...
	Private Function GetCostPerShot() As Double
		Dim C As Short ' technology modifier
		Dim T As Short 'tech level modifier
		Dim I As Short
		...
		GetCostPerShot = System.Math.Round(TempCPS, 2)
	End Function
