Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsGroup_NET.clsGroup")> Public Class clsGroup
	...
	Private mvarLocParent As String
	...
	Public Property Abbrev() As String
		Get
			...
			Abbrev = Veh.Components(mvarParent).Abbrev
		End Get
		Set(ByVal Value As String)
			mvarAbbrev = Value
		End Set
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Groups can only be placed in a subassembly or another Group.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		mvarPrintOutput = "TL" & mvarTL & " " & mvarCustomDescription & ". " & mvarComment
