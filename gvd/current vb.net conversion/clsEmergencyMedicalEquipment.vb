Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsEmergencyMedicalEquipment_NET.clsEmergencyMedicalEquipment")> Public Class clsEmergencyMedicalEquipment
	...
	Private mvarOperatingTables As Integer
	...
	Public Property OperatingTables() As Integer
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.OperatingTables
			OperatingTables = mvarOperatingTables
		End Get
		Set(ByVal Value As Integer)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.OperatingTables = 5
			mvarOperatingTables = Value
		End Set
	End Property
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Emergency and Medical equipment must be placed in Body, Superstructure, Turret, Popturret, Pod ,equipment Pod or Module.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case OperatingRoom
				...
				mvarOperatingTables = 1
			Case StretcherPallet
				
			Case EmergencySupportUnit
				
			Case EmergencylightsandSiren
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(MedicalMatrix)
			If MedicalMatrix(i).ID = mvarDatatype Then
				If MedicalMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		Dim TableMod As Short
		If mvarDatatype = OperatingRoom Then
			TableMod = OperatingTables
		Else
			TableMod = 1
		End If
		
		'get base stats
		mvarWeight = TableMod * MedicalMatrix(mvarMatrixPos).Weight
		mvarCost = TableMod * MedicalMatrix(mvarMatrixPos).Cost
		mvarVolume = TableMod * MedicalMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = System.Math.Round(CDbl(TableMod * mvarQuantity * MedicalMatrix(mvarMatrixPos).Power), 2)
		...
		If mvarOperatingTables <> 0 Then
			sPrint1 = sPrint1 & VB6.Format(mvarOperatingTables) & " table "
		End If
