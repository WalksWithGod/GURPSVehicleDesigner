Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsDuctedFan_NET.clsDuctedFan")> Public Class clsDuctedFan
	...
	Private mvarHoverFan As Boolean
	...
	Public Property LiftEngine() As Boolean
		...
		Set(ByVal Value As Boolean)
			...
			'    If mvarHoverFan And mvarLiftEngine Then
			'        InfoPrint 1, "Lift Engine is incompatible with the Hover Fan option."
	...
	Public Property HoverFan() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.HoverFan
			HoverFan = mvarHoverFan
		End Get
		Set(ByVal Value As Boolean)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.HoverFan = 5
			...
			'    If mvarLiftEngine And mvarHoverFan Then
			'        InfoPrint 1, "Hover Fan is incompatible with the Lift Engine option."
			'        mvarHoverFan = False
			'    End If
			'JAW 2000.06.19
			If mvarHoverFan Then
				If mvarLiftEngine Or mvarVectoredThrust Then
					Exit Property
				Else
					mvarLiftEngine = True
				End If
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Or (InstallPoint = Wing) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Ducted Fans must be placed in Body ,Pod or Wings")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		mvarCost = AirscrewMatrix(mvarMatrixPos).Cost * mvarWeight
		mvarVolume = mvarWeight / AirscrewMatrix(mvarMatrixPos).Volume
		...
		mvarMotiveThrust = System.Math.Round(mvarQuantity * mvarMotivePower * AirscrewMatrix(mvarMatrixPos).MotiveThrust, 2)
		mvarPowerReqt = System.Math.Round(mvarQuantity * mvarMotivePower, 2)
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & VB6.Format(mvarMotivePower, p_sFormat) & " kW " & sPrint1 & mvarCustomDescription & sPrint2 & sPrintPlural & sPrint3 & sPrintPlural2 & VB6.Format(mvarMotiveThrust, p_sFormat) & " lbs. thrust" & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
