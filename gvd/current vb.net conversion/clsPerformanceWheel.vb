Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsPerformanceWheel_NET.clsPerformanceWheel")> Public Class clsPerformanceWheel
	...
	Public Sub CalcPerformance()
		...
		If mvarMotiveAssemblyKey = "" Then
			modHelper.InfoPrint(0, "No Wheel Subassembly detected on vehicle.  Cannot caclulate Wheeled Ground performance.")
			Exit Sub
		Else
		...
		Call GetVehicleWeight(PERFORMANCEWHEEL, mvarPercentAuxVehicleWeight, mvarPercentCargoWeight, mvarPercentAmmunitionWeight, mvarPercentHardpointWeight, mvarPercentFuelWeight, mvarPercentProvisionWeight, m_VWeight, m_VMass)
	...
	Function CalcGroundSpeed() As Single
		...
		For i = 1 To UBound(mvarKeyChain)
			...
			Select Case dType
				Case CStr(WheeledDrivetrain), CStr(AllWheelDriveWheeledDrivetrain), CStr(RopeHarness), CStr(YokeandPoleHarness), CStr(ShaftandCollarHarness), CStr(WhiffletreeHarness)
