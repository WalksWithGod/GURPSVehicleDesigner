Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsSolarPanel_NET.clsSolarPanel")> Public Class clsSolarPanel
	...
	Private mvarSurfaceArea As Double 'THIS IS A SINGLE since the user enters the surface area
	...
	Private mvarRetractable As Boolean
	...
	Public Property Retractable() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.Retractable
			Retractable = mvarRetractable
		End Get
		Set(ByVal Value As Boolean)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.Retractable = 5
			mvarRetractable = Value
		End Set
	End Property
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Wing) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Solar Panels must be placed on Body, Superstructure, Turret, Popturret, or Wing.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		temparea = mvarSurfaceArea
		
		
		If mvarRetractable Then
			' Calculate the cost
			mvarCost = 2 * BasicDesignCost(temparea, mvarTL, mvarFrameStrength, mvarMaterials, mvarResponsive, mvarRobotic, mvarBiomechanical, mvarLivingMetal)
			' Calculate the weight
			mvarWeight = 2 * BasicDesignWeight(temparea, mvarTL, mvarFrameStrength, mvarMaterials)
			' Calculate the component volume
			mvarVolume = ((mvarWeight + GetChildWeight) / 50) * mvarQuantity
		Else
			' Calculate the cost
			mvarCost = BasicDesignCost(temparea, mvarTL, mvarFrameStrength, mvarMaterials, mvarResponsive, mvarRobotic, mvarBiomechanical, mvarLivingMetal)
			' Calculate the weight
			mvarWeight = BasicDesignWeight(temparea, mvarTL, mvarFrameStrength, mvarMaterials)
			' Calculate the component volume
			mvarVolume = 0
		End If
		...
		mvarHitPoints = CalcHitPoints(TypeName(Me), mvarFrameStrength, temparea)
		
		'generate print output
		If mvarRetractable Then sPrint1 = "retractable "
		If mvarQuantity > 1 Then sPrintPlural1 = "s"
		mvarPrintOutput = NumericToString(mvarQuantity) & " " & sPrint1 & mvarCustomDescription & sPrintPlural1 & " (" & mvarLocation & ")."
		
	End Sub
	
	
	Private Function GetChildWeight() As Single
		Dim element As Object
		Dim TempWeight As Single
		
		'todo: here id either already have a reference to the parent OR, the itterator
		' would allow me Itterator.FindLogicalParent(childKey)
