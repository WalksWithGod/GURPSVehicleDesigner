Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsHardPoint_NET.clsHardPoint")> Public Class clsHardPoint
	...
	Public Property LoadCapacity() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Function LocationCheck() As Boolean
		...
		If mvarDatatype = WeaponBay Then
			...
			If (Veh.Components(Parent).Datatype = Body) Then
				TempCheck = True
			Else
				modHelper.InfoPrint(1, "Weapon Bays must be installed in the Body")
				TempCheck = False
			End If
		Else
			'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			If (Veh.Components(Parent).Datatype = Body) Or (Veh.Components(Parent).Datatype = Wing) Or (Veh.Components(Parent).Datatype = Arm) Then
				TempCheck = True
			Else
				modHelper.InfoPrint(1, "Hardpoints must be installed in the Body, Wing or Arm")
				TempCheck = False
			End If
		End If
	...
	Public Sub StatsUpdate()
		...
		mvarWeight = System.Math.Round(mvarQuantity * HardpointMatrix(mvarMatrixPos).Weight * mvarLoadCapacity, 2)
		mvarCost = System.Math.Round(mvarQuantity * HardpointMatrix(mvarMatrixPos).Cost * mvarLoadCapacity, 2)
		If mvarDatatype = WeaponBay Then
			mvarVolume = System.Math.Round(mvarQuantity * HardpointMatrix(mvarMatrixPos).Volume * mvarLoadCapacity, 2)
		...
		End If
		If mvarDatatype = WeaponBay Then
			mvarPrintOutput = NumericToString(mvarQuantity) & sPrintPlural3 & "rated " & VB6.Format(mvarLoadCapacity, p_sFormat) & " lbs." & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
		Else
			mvarPrintOutput = NumericToString(mvarQuantity) & sPrintPlural3 & "rated " & VB6.Format(mvarLoadCapacity, p_sFormat) & " lbs." & " (" & mvarLocation & ", " & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
		End If
