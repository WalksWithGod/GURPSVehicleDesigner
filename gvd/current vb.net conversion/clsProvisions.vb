Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsProvisions_NET.clsProvisions")> Public Class clsProvisions
	...
	Private mvarOccupancyDays As Integer
	...
	Public Property occupancydays() As Integer
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.OccupancyDays
			occupancydays = mvarOccupancyDays
		End Get
		Set(ByVal Value As Integer)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.OccupancyDays = 5
			mvarOccupancyDays = Value
		End Set
	End Property
	...
	Public Property Setting() As String
		...
		Set(ByVal Value As String)
			...
			'Syntax: X.Setting = 5
			mvarSetting = Value
			If mvarSetting = "" Then mvarSetting = "auto"
		End Set
	End Property
	...
	Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Provisions must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, or Leg.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		Dim tcost, TWeight As Object
		Dim TVolume As Single
		Dim element As Object
		Dim LifeSystems As Boolean
		Dim lngDatatype As Integer
		
		mvarLocation = GetLocation
		
		'note: wastefull loop here
		If mvarSetting = "auto" Then
			mvarMatrixPos = 1
			'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			For	Each element In Veh.Components
				...
				lngDatatype = element.Datatype
				Select Case lngDatatype
					Case FullLifeSystem, LimitedLifeSystem, TotalLifeSystem
						mvarMatrixPos = 2
						Exit For
				End Select
			Next element
		ElseIf mvarSetting = "light" Then 
			mvarMatrixPos = 2
		Else
			mvarMatrixPos = 1 '//heavy setting
			
		End If
		
		'UPGRADE_WARNING: Couldn't resolve default property of object tcost. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		tcost = ProvisionsMatrix(mvarMatrixPos).Cost
		'UPGRADE_WARNING: Couldn't resolve default property of object TWeight. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		TWeight = ProvisionsMatrix(mvarMatrixPos).Weight
		TVolume = ProvisionsMatrix(mvarMatrixPos).Volume
		
		
		'UPGRADE_WARNING: Couldn't resolve default property of object TWeight. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarWeight = System.Math.Round(mvarOccupancyDays * TWeight, 2)
		'UPGRADE_WARNING: Couldn't resolve default property of object tcost. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarCost = System.Math.Round(mvarOccupancyDays * tcost, 2)
		mvarVolume = System.Math.Round(mvarOccupancyDays * TVolume, 2)
		...
		'print output
		mvarPrintOutput = VB6.Format(mvarOccupancyDays) & " days worth of " & mvarCustomDescription & " (" & mvarLocation & ", HP " & mvarHitPoints & ", " & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
