Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsShields_NET.clsShields")> Public Class clsShields
	...
	Private mvarForceDR As Integer
	Private mvarPDBonus As Short
	...
	Public Property ForceDR() As Integer
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.ForceDR
			ForceDR = mvarForceDR
		End Get
		Set(ByVal Value As Integer)
			'used when assigning a value to the property, on the left side of an assignment.
			...
			If mvarForceDR < 100 Then
				mvarForceDR = 100
				modHelper.InfoPrint(1, "Minimum Force DR is 100")
			End If
	...
	Public Property PDBonus() As Integer
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.PDBonus
			PDBonus = mvarPDBonus
		End Get
		Set(ByVal Value As Integer)
			'used when assigning a value to the property, on the left side of an assignment.
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Wing) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Field Grids must be placed in Body, Superstructure, Pod, Turret, Popturret, Wing or Module.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(ShieldsMatrix)
			If ShieldsMatrix(i).ID = mvarDatatype Then
				If ShieldsMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		sf = Veh.Stats.totalSurfaceArea
		' Calculate the Weight
		If mvarDatatype = DeflectorField Then
			mvarWeight = System.Math.Round(ShieldsMatrix(mvarMatrixPos).Weight * sf, 2)
			mvarCost = System.Math.Round(ShieldsMatrix(mvarMatrixPos).Cost * sf, 2)
			mvarPowerReqt = System.Math.Round(ShieldsMatrix(mvarMatrixPos).Power * sf, 2)
		Else
			mvarWeight = System.Math.Round(mvarForceDR * ShieldsMatrix(mvarMatrixPos).Weight * sf, 2)
			mvarCost = System.Math.Round(mvarForceDR * ShieldsMatrix(mvarMatrixPos).Cost * sf, 2)
			mvarPowerReqt = System.Math.Round(mvarForceDR * ShieldsMatrix(mvarMatrixPos).Power * sf, 2)
		End If
		mvarVolume = 0
		...
		If mvarForceDR <> 0 Then
			sPrint1 = ", DR +" & VB6.Format(mvarForceDR)
		ElseIf mvarPDBonus <> 0 Then 
			sPrint1 = ", PD +" & VB6.Format(mvarPDBonus)
		End If
		
		mvarPrintOutput = " TL" & mvarTL & " " & mvarCustomDescription & " (" & mvarLocation & ", HP " & mvarHitPoints & ", " & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW" & sPrint1 & ")." & mvarComment
