Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsMuscleEngine_NET.clsMuscleEngine")> Public Class clsMuscleEngine
	
	Private mvarTL As Short
	Private mvarWeight As Double
	Private mvarVolume As Double
	Private mvarCost As Double
	Private mvarMaxOutput As Single
	Private mvarOutput As Single
	Private mvarCombinedST As Integer
	...
	Public Property CombinedST() As Integer
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Property MaxOutput() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.Output
			MaxOutput = mvarMaxOutput
		End Get
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Or (InstallPoint = Wing) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Superstructure) Or (InstallPoint = Arm) Or (InstallPoint = Leg) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Muscle Engines must go in the Body, Pod, Wing, Turret, Popturret, Superstructure, Arm or Leg.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(MuscleEngineMatrix)
			If MuscleEngineMatrix(i).ID = mvarDatatype Then
				If MuscleEngineMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = mvarMaxOutput * MuscleEngineMatrix(mvarMatrixPos).Weight
		mvarCost = mvarMaxOutput * MuscleEngineMatrix(mvarMatrixPos).Cost
		mvarVolume = mvarMaxOutput * MuscleEngineMatrix(mvarMatrixPos).Volume
		...
		TempOutput = MuscleEngineMatrix(mvarMatrixPos).Output * mvarCombinedST
		
		'make sure the output cant exceed maxoutput
		...
		TempOutput = modPerformance.Minimum(mvarMaxOutput, TempOutput)
		...
		mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
		mvarOutput = System.Math.Round(mvarQuantity * TempOutput, 2)
