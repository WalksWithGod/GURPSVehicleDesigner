Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsRotor_NET.clsRotor")> Public Class clsRotor
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = Pod) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Rotors must be placed on the Body, Superstructure, Turret or a Pod.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		' Calculate the volume of the Rotors.
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarVolume = System.Math.Round(0.02 * Veh.Components(BODY_KEY).Volume, 2)
		' calculate the surface area
		mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
		
		' apply the surface area multiplier of x3 for rotors
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarPrintOutput = CStr(CDbl(mvarCustomDescription & " (") + Veh.Components(Parent).CustomDescription + CDbl(sPrint2) + CDbl(")."))
	...
	Public Sub CalcWingRotorOptionWeightsAndCosts()
		...
		Divisor = structarea * structweight
		
		'folding cost and weight
		If (mvarFolding) And (Divisor <> 0) Then
			mvarFoldingWeight = 0.05 * mvarSurfaceArea / structarea * structweight
