Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsTurret_NET.clsTurret")> Public Class clsTurret
		...
		Set(ByVal Value As String)
			...
			If TypeOf Veh.Components(mvarParent) Is clsBody Then
				If mvarRotation = "Full" Then
					If (Value <> "top") And (Value <> "underside") Then
						Value = "top"
						modHelper.InfoPrint(1, "Full Rotation turrets can only be placed on the top or underside of its supporting structure")
					Else
						mvarOrientation = Value
					End If
				ElseIf mvarRotation = "Limited" Then 
					If (Value = "top") And (Value = "underside") Then
						Value = "front"
						modHelper.InfoPrint(1, "Limited Rotation turrets can only be placed on the Front, Back, Left or Right sides of its supporting structure")
					Else
						mvarOrientation = Value
					End If
				Else
			...
			ElseIf TypeOf Veh.Components(mvarParent) Is clsTurret Then 
				...
				' MPJ 11/7/2000
				' this has been removed as a hard restriction and
				' placed into the design check instead
				'If Temp = vdata Then
				mvarOrientation = Value
				'Else
				'    mvarOrientation = Temp
				'    InfoPrint 1, "Stacked Turrets must have the same orientation"
				'End If
				'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			ElseIf TypeOf Veh.Components(mvarParent) Is clsSuperStructure Then 
				...
				Else
					...
					Temp = Veh.Components(mvarParent).Orientation
					mvarOrientation = Temp
					modHelper.InfoPrint(1, "Turrets placed on Superstructures must have same orientation as that Superstructure")
				End If
			End If
	...
	Public Property compartmentalizationcost() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = Turret) Or (InstallPoint = Superstructure) Or (InstallPoint = Arm) Or (InstallPoint = Leg) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Turrets must be placed on Body, Superstructure, Leg, Arm or another Turret.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		mvarVolume = mvarVolume + CalcCombinedVolume(mvarKey) + (mvarEmptySpace * mvarQuantity) + (mvarAccessSpace * mvarQuantity)
		...
		If mvarBattlesuitVolumeAdded Then
			...
			If Veh.Options.BattleSuit = "Form Fitting" Then
				'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				mvarVolume = mvarVolume + Veh.Components("2_").Volume2
			Else
				'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				mvarVolume = mvarVolume + Veh.Components("2_").Volume1
			End If
		End If
		...
		If mvarRotation = "full" Then
			mvarRotationSpace = mvarVolume * 0.2
		ElseIf Rotation = "limited" Then 
			mvarRotationSpace = mvarVolume * 0.1
		Else
			mvarRotationSpace = 0
		End If
