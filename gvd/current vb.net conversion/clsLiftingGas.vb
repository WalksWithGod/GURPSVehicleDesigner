Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsLiftingGas_NET.clsLiftingGas")> Public Class clsLiftingGas
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Gasbag) Or (InstallPoint = Pod) Or (InstallPoint = Body) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Lifting Gas must be placed in Gasbag, Body or Pods.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(LiftingGasMatrix)
			If LiftingGasMatrix(i).ID = mvarDatatype Then
				If LiftingGasMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'    mvarVolume = 1.5 * mvarLift * LiftingGasMatrix(mvarMatrixPos).Volume
		'    mvarCost = 1.5 * mvarVolume * LiftingGasMatrix(mvarMatrixPos).Cost
		'JAW 2000.06.19
		'removed mysterious multipliers to correct lifting gas vol and cost
		mvarVolume = mvarLift * LiftingGasMatrix(mvarMatrixPos).Volume
		mvarCost = mvarVolume * LiftingGasMatrix(mvarMatrixPos).Cost
		
		mvarPrintOutput = VB6.Format(mvarLift, p_sFormat) & " lb. lift " & mvarCustomDescription & " (" & mvarLocation & ", " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
