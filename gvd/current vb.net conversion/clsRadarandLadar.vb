Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsRadarandLadar_NET.clsRadarandLadar")> Public Class clsRadarandLadar
	
	Private mvarNoTargeting As Boolean
	Private mvarSearchOption As String
	...
	Public Property SearchOption() As String
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.SearchOption
			SearchOption = mvarSearchOption
		End Get
		Set(ByVal Value As String)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.SearchOption = 5
			mvarSearchOption = Value
		End Set
	End Property
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(RadarMatrix)
			If RadarMatrix(i).ID = mvarDatatype Then
				If RadarMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		Const FTL_SCAN_BOOST As Short = 32
		...
		'Cost Exception for Ranges greater than 125 miles
		If mvarRange > 125 Then
			mvarCost = CostMod * RadarMatrix(mvarMatrixPos).Cost * (100 + (1 / 5 * mvarRange))
		Else
			mvarCost = CostMod * mvarRange * RadarMatrix(mvarMatrixPos).Cost
		End If
		'get base stats
		mvarWeight = WeightMod * RadarMatrix(mvarMatrixPos).Weight * Range
		mvarVolume = VolumeMod * mvarRange * RadarMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = RadarMatrix(mvarMatrixPos).Power * PowerMod * mvarRange
		
		If mvarFTL Then
			mvarScanRating = mvarScanRating + FTL_SCAN_BOOST
		End If
		...
		If mvarSearchOption <> "none" Then
			If sPrint2 = "" Then
				sPrint2 = ", " & mvarSearchOption
			Else
				sPrint2 = sPrint2 & ", " & mvarSearchOption & " option"
			End If
		End If
		If mvarFTL Then
			If sPrint2 = "" Then
				sPrint2 = ", " & "FTL option"
			Else
				sPrint2 = sPrint2 & ", and FTL options"
			End If
			sPrint3 = " light secs range"
		Else
			sPrint3 = " mile range"
		End If
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & sPrintPlural & ", " & VB6.Format(mvarRange, p_sFormat) & sPrint3 & sPrint2 & " (" & mvarLocation & sPrintDirection & ", HP " & mvarHitPoints & sPrintPlural2 & ", " & sPrintPlural3 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW" & ", Scan " & VB6.Format(mvarScanRating) & ")." & mvarComment
