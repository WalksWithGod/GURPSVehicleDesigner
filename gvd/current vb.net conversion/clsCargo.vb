Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsCargo_NET.clsCargo")> Public Class clsCargo
	...
	Private mvarWeightPerCubicFoot As Object
	...
	Public Property CargoSpace() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Property CargoWeight() As Double
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Property WeightPerCubicFoot() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.CargoWeight
			'UPGRADE_WARNING: Couldn't resolve default property of object mvarWeightPerCubicFoot. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Cargo must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount or Leg.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		If mvarSubType = "hidden" Then
			mvarVolume = System.Math.Round(mvarCargoSpace, 2)
			mvarCost = 20 * mvarVolume
		ElseIf mvarSubType = "open" Then 
			mvarVolume = System.Math.Round(mvarCargoSpace / 2, 2)
			mvarCost = 0
		Else
			mvarCost = 0
			mvarVolume = System.Math.Round(mvarCargoSpace, 2)
		...
		If mvarSubType <> "standard" Then
			sPrint1 = " " & mvarSubType
		End If
		mvarPrintOutput = VB6.Format(mvarCargoSpace, p_sFormat) & " cf" & sPrint1 & " " & mvarCustomDescription & " (" & mvarLocation & ")."
	End Sub
