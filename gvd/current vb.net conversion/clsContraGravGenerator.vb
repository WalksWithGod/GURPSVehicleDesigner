Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsContraGravGenerator_NET.clsContraGravGenerator")> Public Class clsContraGravGenerator
	...
	Private mvarDesiredLift As Single
	...
	Public Property DesiredLift() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.DesiredLift
			DesiredLift = mvarDesiredLift
		End Get
		Set(ByVal Value As Single)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.DesiredLift = 5
			mvarDesiredLift = Value
		End Set
	End Property
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Pod) Or (InstallPoint = Body) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Contragrav Generators must be placed in Body or Pods")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(ContraGravMatrix)
			If ContraGravMatrix(i).ID = mvarDatatype Then
				If ContraGravMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		TempWeight = (mvarDesiredLift / ContraGravMatrix(mvarMatrixPos).Weight1) + ContraGravMatrix(mvarMatrixPos).Weight2
		TempCost = (mvarDesiredLift / ContraGravMatrix(mvarMatrixPos).Cost1) + ContraGravMatrix(mvarMatrixPos).Cost2
		TempVolume = TempWeight / ContraGravMatrix(mvarMatrixPos).Volume
		...
		mvarLift = mvarDesiredLift * mvarQuantity
		...
		mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
		mvarPowerReqt = System.Math.Round(mvarLift / ContraGravMatrix(mvarMatrixPos).Power, 2)
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & VB6.Format(mvarDesiredLift, p_sFormat) & " lb. lift " & sPrint1 & mvarCustomDescription & sPrintPlural & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
