Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsBeamedPowerReceiver_NET.clsBeamedPowerReceiver")> Public Class clsBeamedPowerReceiver
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Beamed Power Receivers must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case LaserBeamedPowerReceiver
				
			Case MaserBeamedPowerReceiver
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(BeamedPowerMatrix)
			If BeamedPowerMatrix(i).ID = mvarDatatype Then
				If BeamedPowerMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		TrueWeight = mvarDesiredOutput * BeamedPowerMatrix(mvarMatrixPos).Weight
		
		'Find the volume
		TempVolume = mvarDesiredOutput * BeamedPowerMatrix(mvarMatrixPos).Volume
		
		'make sure minimum cost is met
		TempCost = (mvarDesiredOutput * BeamedPowerMatrix(mvarMatrixPos).Cost) + BeamedPowerMatrix(mvarMatrixPos).BaseCost
		
		'find the base stats
		mvarWeight = TrueWeight
		mvarCost = TempCost
		mvarVolume = TempVolume
		mvarSurfaceArea = CalcSurfaceArea(TempVolume)
