Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsAquaticPropulsion_NET.clsAquaticPropulsion")> Public Class clsAquaticPropulsion
	...
	Public Function LocationCheck() As Boolean
		...
		Select Case mvarDatatype
			Case PaddleWheel
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Paddle Wheels must be placed in the Vehicles Body")
					TempCheck = False
				End If
			Case ScrewPropeller, lightScrewPropeller, DuctedPropeller, MHDTunnel
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Propellers and MHD Tunnels must be placed in Body or Pods")
					TempCheck = False
				End If
			Case Hydrojet
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Or (InstallPoint = Hydrofoil) Or (InstallPoint = Leg) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Hydrojets must be placed in Body, Pods, Hydrofoil or Legs")
					TempCheck = False
				End If
		End Select
	...
	Public Sub Init()
		Select Case mvarDatatype
			...
			Case lightScrewPropeller
				
			Case DuctedPropeller
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(AquaticMatrix)
			If AquaticMatrix(i).ID = mvarDatatype Then
				If AquaticMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		If MotivePower >= 5 Then
			TempWeight1 = AquaticMatrix(mvarMatrixPos).Weight2
			TempWeight2 = AquaticMatrix(mvarMatrixPos).Weight3
		Else
			TempWeight1 = AquaticMatrix(mvarMatrixPos).Weight1
			TempWeight2 = 0
		End If
		
		' Calculate the Weight
		
		mvarWeight = RugMod * ((mvarMotivePower * TempWeight1) + TempWeight2)
		mvarCost = mvarWeight * AquaticMatrix(mvarMatrixPos).Cost
		mvarVolume = mvarWeight / AquaticMatrix(mvarMatrixPos).Volume
		...
		mvarMotiveThrust = mvarMotivePower * AquaticMatrix(mvarMatrixPos).MotiveThrust
		...
		mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
		mvarMotiveThrust = System.Math.Round(mvarMotiveThrust * mvarQuantity, 2)
		mvarPowerReqt = System.Math.Round(mvarMotivePower * mvarQuantity, 2)
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & VB6.Format(mvarMotivePower, p_sFormat) & " kW " & sPrint1 & mvarCustomDescription & sPrintPlural & sPrintPlural2 & VB6.Format(mvarMotiveThrust, p_sFormat) & " lbs. aquatic thrust" & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
