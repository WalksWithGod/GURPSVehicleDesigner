Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsTeleportProjector_NET.clsTeleportProjector")> Public Class clsTeleportProjector
	...
	Private mvarHexCapacity As Integer
	...
	Public Property HexCapacity() As Integer
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.HexCapacity
			HexCapacity = mvarHexCapacity
		End Get
		Set(ByVal Value As Integer)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.HexCapacity = 5
			mvarHexCapacity = Value
		End Set
	End Property
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Teleport Projectors must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(TeleporterMatrix)
			If TeleporterMatrix(i).ID = mvarDatatype Then
				If TeleporterMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = mvarHexCapacity * TeleporterMatrix(mvarMatrixPos).Weight
		mvarCost = mvarHexCapacity * TeleporterMatrix(mvarMatrixPos).Cost
		mvarVolume = mvarHexCapacity * TeleporterMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = mvarHexCapacity * TeleporterMatrix(mvarMatrixPos).Power
