Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsNavigationSystem_NET.clsNavigationSystem")> Public Class clsNavigationSystem
	
	Private mvarPrecision As Boolean
	...
	Public Property Precision() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Function LocationCheck() As Boolean
		...
		ElseIf (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then 
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Navigation Instruments and Electronics must be placed in Periscope, Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(NavigationSystemMatrix)
			If NavigationSystemMatrix(i).ID = mvarDatatype Then
				If NavigationSystemMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = NavigationSystemMatrix(mvarMatrixPos).Weight
		mvarCost = PrecisionMod * NavigationSystemMatrix(mvarMatrixPos).Cost
		mvarVolume = NavigationSystemMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = NavigationSystemMatrix(mvarMatrixPos).Power
		'get finals
		...
		Else
			sPrint1 = ""
		End If
		If mvarPrecision Then
			sPrint1 = sPrint1 & "precision "
		End If
