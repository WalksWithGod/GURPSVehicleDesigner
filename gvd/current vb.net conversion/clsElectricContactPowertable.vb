Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsElectricContactPower_NET.clsElectricContactPower")> Public Class clsElectricContactPower
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Superstructure) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Electric Contact Power Plants must go in Body, Turret, Popturret, or Superstructure.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(ElectricContactMatrix)
			If ElectricContactMatrix(i).ID = mvarDatatype Then
				If ElectricContactMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		TrueWeight = mvarDesiredOutput * ElectricContactMatrix(mvarMatrixPos).Weight
		
		'Find the volume
		TempVolume = TrueWeight / ElectricContactMatrix(mvarMatrixPos).Volume
		
		'make sure minimum cost is met (this is done BEFORE closed cycle calcs)
		TempCost = TrueWeight * ElectricContactMatrix(mvarMatrixPos).Cost
