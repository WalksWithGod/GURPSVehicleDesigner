Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsJetEngine_NET.clsJetEngine")> Public Class clsJetEngine
	...
	Public Property Afterburner() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Property ABThrust() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Property ABConsumption() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Pod) Or (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Wing) Or (InstallPoint = Superstructure) Or (InstallPoint = Leg) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Jet engines must be placed in Body, Wing, Pod, Superstructure or Leg")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		If mvarMotiveThrust = 0 Then
			mvarOutput = 0
		Else
			mvarOutput = System.Math.Round(mvarMotiveThrust / 100, 2)
		End If
		...
		If mvarAfterburner Then
			...
			End If
			sPrint4 = " " & VB6.Format(mvarABThrust, p_sFormat) & " with afterburner"
			sPrint5 = " " & VB6.Format(mvarABConsumption, p_sFormat) & " with afterburner"
			
			If mvarQuantity > 1 Then sPrint3 = sPrint3 & "s"
		End If
		
		If mvarDatatype = FusionAirRam Then
			sPrint6 = " has total operational life of " & VB6.Format(mvarFuelConsumption, p_sFormat) & " yrs"
		Else
			sPrint6 = " uses " & VB6.Format(mvarFuelConsumption, p_sFormat) & " gph jet fuel" & sPrint5
		End If
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & sPrint2 & sPrintPlural & sPrint3 & sPrintPlural2 & VB6.Format(mvarMotiveThrust, p_sFormat) & " lbs. motive thrust" & sPrint4 & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")" & sPrint6 & "." & mvarComment
