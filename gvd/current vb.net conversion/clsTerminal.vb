Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsTerminal_NET.clsTerminal")> Public Class clsTerminal
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Terminals must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(TerminalMatrix)
			If TerminalMatrix(i).ID = mvarDatatype Then
				If TerminalMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = TerminalMatrix(mvarMatrixPos).Weight
		mvarCost = TerminalMatrix(mvarMatrixPos).Cost
		mvarVolume = TerminalMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = TerminalMatrix(mvarMatrixPos).Power
