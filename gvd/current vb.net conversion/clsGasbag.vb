Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsGasbag_NET.clsGasbag")> Public Class clsGasbag
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = Superstructure) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Gasbags must be placed on Body or Superstructure.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		' Volume for Gasbag is equal to amount of lifting gas attached
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		For i = 1 To Veh.Components.count
			'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			If Veh.Components(i).LogicalParent = mvarKey Then
				'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				...
				If TypeName(Veh.Components(i)) = "clsLiftingGas" Then
					'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					mvarVolume = Veh.Components(i).Volume
					Exit For
				End If
			End If
		Next 
		...
		' Calculate the cost using special Gasbag rules
		Select Case Val(CStr(mvarTL))
			Case Is >= 7
				mvarCost = mvarSurfaceArea * 0.01
			Case Is <= 5
				mvarCost = mvarSurfaceArea * 0.003
			Case 6
				mvarCost = mvarSurfaceArea * 0.005
		End Select
		
		' Calculate the weight using special Gasbag rules
		...
		mvarWeight = (mvarSurfaceArea * TechLevelModifier(mvarTL)) / 1000
		' Calculate the Hit Points using Gasbag specifc rules.
		...
		mvarHitPoints = CalcComponentHitpoints(mvarSurfaceArea * 0.01)
		' make sure that we always have at least 1 hit point... can't have 0
		If mvarHitPoints < 1 Then
			mvarHitPoints = 1
		End If
		
		'generate print output
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarPrintOutput = CStr(CDbl(mvarCustomDescription & sPrintPlural1 & " " & " (on ") + Veh.Components(mvarParent).CustomDescription + CDbl(")."))
