Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsNuclearDamper_NET.clsNuclearDamper")> Public Class clsNuclearDamper
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Or (InstallPoint = Leg) Or (InstallPoint = Arm) Or (InstallPoint = OpenMount) Or (InstallPoint = Wing) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Nuclear Dampers must be placed in Body, Superstructure, Turret, Popturret,  Open Mount, Pod, equipment Pod, Wing, Leg, Arm or Module.")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(NuclearDamperMatrix)
			If NuclearDamperMatrix(i).ID = mvarDatatype Then
				If NuclearDamperMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = Rmod * NuclearDamperMatrix(mvarMatrixPos).Weight
		mvarCost = Rmod * NuclearDamperMatrix(mvarMatrixPos).Cost
		mvarVolume = Rmod * NuclearDamperMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = mvarRadius * NuclearDamperMatrix(mvarMatrixPos).Power
		...
		If mvarRadius <> 0 Then
			sPrint2 = ", " & VB6.Format(mvarRadius, p_sFormat) & " mile radius"
