Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsMast_NET.clsMast")> Public Class clsMast
	...
	Public Property DependantStructure() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Property Height() As Integer
		...
		Set(ByVal Value As Integer)
			...
			If mvarHeight > MaxHeight Then
				modHelper.InfoPrint(1, "Maximum Mast height at this tech level using " & mvarMaterials & " materials is " & MaxHeight & " ft.")
	...
	Public Property TL() As Short
		...
		Set(ByVal Value As Short)
			...
			If (mvarTL < 5) And (mvarMaterials = "metal") Then
				modHelper.InfoPrint(1, "Metal masts unavailable prior to tech level 5. Material reset to 'wood'.")
				mvarMaterials = "wood"
			End If
	...
	Public Property Materials() As String
		...
		Set(ByVal Value As String)
			...
			If (mvarMaterials = "metal") And (mvarTL < 5) Then
				modHelper.InfoPrint(1, "Metal masts not allowed prior to tech level 5.")
				mvarMaterials = "wood"
			End If
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = Superstructure) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Masts must be placed on Body or Superstructure.")
			TempCheck = False
		End If
	...
	Public Sub StatsUpdate()
		...
		mvarVolume = System.Math.Round(((mvarHeight ^ 3) / 10000) * mvarQuantity, 2)
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarPrintOutput = CStr(CDbl(NumericToString(mvarQuantity) & " " & VB6.Format(mvarHeight) & " ft. " & mvarMaterials & " " & mvarCustomDescription & sPrintPlural1 & " " & " (atop ") + Veh.Components(mvarParent).CustomDescription + CDbl(")."))
