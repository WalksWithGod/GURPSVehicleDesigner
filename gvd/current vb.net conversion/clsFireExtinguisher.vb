Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsFireExtinguisher_NET.clsFireExtinguisher")> Public Class clsFireExtinguisher
	...
	Public Function LocationCheck() As Boolean
		...
		If InstallPoint = Body Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Fire Extinguishers must be placed in the hull.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case FireExtinguisherSystem
				
			Case FullFireSuppressionSystem
				
			Case CompactFireSuppressionSystem
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(FireExtinguisherMatrix)
			If FireExtinguisherMatrix(i).ID = mvarDatatype Then
				If FireExtinguisherMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		mvarWeight = FireExtinguisherMatrix(mvarMatrixPos).Weight
		mvarCost = FireExtinguisherMatrix(mvarMatrixPos).Cost
		mvarVolume = FireExtinguisherMatrix(mvarMatrixPos).Volume
		...
		Else
			...
			sPrintPlural3 = ""
		End If
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & sPrintPlural & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural2 & ", " & sPrintPlural3 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
