Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsScientificSensor_NET.clsScientificSensor")> Public Class clsScientificSensor
	...
	Public Function LocationCheck() As Boolean
		...
		ElseIf (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then 
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Scientific Instruments and Electronics must be placed in Periscope, Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case LowResPlanetarySurveyArray
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(ScientificSensorMatrix)
			If ScientificSensorMatrix(i).ID = mvarDatatype Then
				If ScientificSensorMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = ScientificSensorMatrix(mvarMatrixPos).Weight
		mvarCost = ScientificSensorMatrix(mvarMatrixPos).Cost
		mvarVolume = ScientificSensorMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = ScientificSensorMatrix(mvarMatrixPos).Power
		'get finals
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & sPrintPlural & " (" & mvarLocation & sPrintDirection & ", HP " & mvarHitPoints & sPrintPlural2 & ", " & sPrintPlural3 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
