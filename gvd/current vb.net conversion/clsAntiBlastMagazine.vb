Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsAntiBlastMagazine_NET.clsAntiBlastMagazine")> Public Class clsAntiBlastMagazine
	...
	Public Function LocationCheck() As Boolean
		...
		If (mvarDatatype = GroupComponent) Or (Veh.Components(Parent).Datatype = Ammunition) Or TypeOf Veh.Components(Parent) Is clsWeaponTorpMissile Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Anti-Blast Magazines must be added to Ammunition or Torpedos and Missiles")
			TempCheck = False
		End If
	...
	Public Sub SetLogicalParent()
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarLogicalParent = GetLogicalParent(Veh.Components(mvarParent).Parent)
	End Sub
	...
	Public Sub StatsUpdate()
		...
		Dim ParentVolume As Single
		
		mvarLocation = GetLocation
		
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		ParentVolume = Veh.Components(mvarParent).Volume
		
		
		mvarWeight = System.Math.Round(WeaponAccessoryMatrix(mvarMatrixPos).Weight * ParentVolume, 2)
		mvarCost = System.Math.Round(WeaponAccessoryMatrix(mvarMatrixPos).Cost * ParentVolume, 2)
		
		'generate print output
		'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		mvarPrintOutput = mvarCustomDescription & " for " & Veh.Components(mvarParent).CustomDescription & " (" & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
