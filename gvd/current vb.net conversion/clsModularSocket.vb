Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsModularSocket_NET.clsModularSocket")> Public Class clsModularSocket
	...
	Public Property RatedVolume() As Double
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Property WasteWeight() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
		...
		Set(ByVal Value As Single)
			...
			mvarWasteWeight = Value
		End Set
	End Property
	
	
	
	Public Property WasteVolume() As Single
		Get
			'used when retrieving value of a property, on the right side of an assignment.
		...
		Select Case mvarDatatype
			Case ModularSocket
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Or (InstallPoint = Leg) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = equipmentPod) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Modular Sockets must be placed in Body, Superstructure, Turret, Popturret, Pod, equipment Pod, Wing, Leg or Arm.")
					TempCheck = False
				End If
			Case Else
				If InstallPoint = ModularSocket Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Modules must be placed in Modular Sockets.")
					TempCheck = False
				End If
		End Select
	...
	Public Sub StatsUpdate()
		...
		If mvarDatatype = ModularSocket Then
			mvarCost = System.Math.Round(ModularSocketMatrix(mvarMatrixPos).Cost, 2)
		...
		Else
			For	Each element In Veh
				...
				End If
			Next element
			mvarVolume = System.Math.Round(TempVolume + mvarWasteVolume, 2)
			mvarWeight = System.Math.Round(TempWeight + mvarWasteWeight, 2)
			mvarCost = System.Math.Round(TempCost + (TempCost * 0.2), 2)
		End If
		
		
		If mvarRatedVolume <> 0 Then
			sPrint2 = ", " & VB6.Format(mvarRatedVolume, p_sFormat) & " cf. rated volume"
		Else
			sPrint2 = ", holds "
			For i = 1 To UBound(socketchildnames)
				sPrint2 = sPrint2 & socketchildnames(i)
				If i <= UBound(socketchildnames) Then sPrint2 = sPrint2 & ", "
			Next 
		...
		mvarPrintOutput = mvarCustomDescription & sPrint2 & " (" & mvarLocation & ", " & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
