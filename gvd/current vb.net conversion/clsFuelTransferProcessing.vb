Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsFuelAccessory_NET.clsFuelAccessory")> Public Class clsFuelAccessory
		...
		Select Case mvarDatatype
			...
			Case RefuellingProbe, RefuellingDrogue
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Or (InstallPoint = Wing) Or (InstallPoint = equipmentPod) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Refuelling Probes and Drogues must be placed in Body, Pod, equipment Pod or Wing.")
					TempCheck = False
				End If
			Case HydrogenFuelScoop
				...
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Wing) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Hydrogen fuel scoops must be placed in Body or Wing.")
					TempCheck = False
				End If
			Case Else
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Or (InstallPoint = equipmentPod) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Fuel Electrolysis Systems and Atmosphere Processors must be placed in Body, Superstructure, Turret, Popturret, Pod or equipment Pod.")
					TempCheck = False
				End If
		End Select
	...
	Public Sub StatsUpdate()
		...
		If mvarCapacity <> 0 Then
			sPrint2 = ", " & VB6.Format(mvarCapacity, p_sFormat) & " gallon capacity"
