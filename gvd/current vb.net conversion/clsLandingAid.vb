Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsLandingAid_NET.clsLandingAid")> Public Class clsLandingAid
	...
	Private mvarRatedWeight As Single
	...
	Public Property RatedWeight() As Double
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.RateDweight
			RatedWeight = mvarRatedWeight
		End Get
		Set(ByVal Value As Double)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.RateDweight = 5
			mvarRatedWeight = Value
		End Set
	End Property
		...
		Select Case mvarDatatype
			...
			Case ArrestorHook
				If InstallPoint = Body Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Arrestor Hooks must be placed in hull.")
					TempCheck = False
				End If
			Case Else
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Vehicular Parachute must be placed in Body, Superstructure, Turret, Popturret or Pod.")
					TempCheck = False
				End If
		End Select
	...
	Public Sub Init()
		Select Case mvarDatatype
			...
			Case VehicularParachute
				
				mvarRatedWeight = 2000
		End Select
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(LandingAidMatrix)
			If LandingAidMatrix(i).ID = mvarDatatype Then
				If LandingAidMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		Select Case mvarDatatype
			Case VehicularParachute
				WMod = mvarRatedWeight / 1000
				mvarWeight = WMod * LandingAidMatrix(mvarMatrixPos).Weight
				mvarCost = WMod * LandingAidMatrix(mvarMatrixPos).Cost
				mvarVolume = WMod * LandingAidMatrix(mvarMatrixPos).Volume
				...
				mvarPowerReqt = WMod * LandingAidMatrix(mvarMatrixPos).Power
				
			Case Else
				mvarWeight = LandingAidMatrix(mvarMatrixPos).Weight
				mvarCost = LandingAidMatrix(mvarMatrixPos).Cost
				mvarVolume = LandingAidMatrix(mvarMatrixPos).Volume
				mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
				'UPGRADE_WARNING: Couldn't resolve default property of object CalcComponentHitpoints(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				mvarHitPoints = CalcComponentHitpoints(RugHitMod * mvarSurfaceArea)
				mvarPowerReqt = LandingAidMatrix(mvarMatrixPos).Power
		End Select
		...
		If mvarRatedWeight <> 0 Then
			sPrint2 = ", " & VB6.Format(mvarRatedWeight, p_sFormat) & " lb rated weight"
			If mvarQuantity > 1 Then sPrint2 = sPrint2 & sPrintPlural2
		Else
			sPrint2 = ""
		End If
		
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & sPrintPlural & sPrint2 & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural2 & ", " & sPrintPlural3 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ")." & mvarComment
