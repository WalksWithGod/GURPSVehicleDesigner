Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsReactionlessThruster_NET.clsReactionlessThruster")> Public Class clsReactionlessThruster
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Pod) Or (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Wing) Or (InstallPoint = Superstructure) Or (InstallPoint = Leg) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Reactionless Thrusters must be placed in Body, Wing, Pod, Superstructure or Leg")
			TempCheck = False
		End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(ReactionlessMatrix)
			If ReactionlessMatrix(i).ID = mvarDatatype Then
				If ReactionlessMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		mvarWeight = ReactionlessMatrix(mvarMatrixPos).Weight * mvarDesiredThrust
		mvarCost = mvarWeight * ReactionlessMatrix(mvarMatrixPos).Cost
		mvarVolume = mvarWeight / ReactionlessMatrix(mvarMatrixPos).Volume
		mvarMotiveThrust = mvarQuantity * mvarDesiredThrust
		'make sure its not less than the minimum cost
		...
		mvarCost = modPerformance.Maximum(mvarCost, ReactionlessMatrix(mvarMatrixPos).MinCost)
		
		'Get Power consumption
		mvarPowerReqt = mvarDesiredThrust * ReactionlessMatrix(mvarMatrixPos).Power
		...
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & VB6.Format(mvarDesiredThrust, p_sFormat) & " lb. thrust " & sPrint1 & mvarCustomDescription & sPrint2 & sPrintPlural & sPrint3 & " (" & mvarLocation & ", HP " & mvarHitPoints & sPrintPlural3 & ", " & sPrintPlural4 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
