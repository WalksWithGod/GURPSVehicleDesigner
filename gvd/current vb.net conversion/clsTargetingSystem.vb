Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsTargetingSystem_NET.clsTargetingSystem")> Public Class clsTargetingSystem
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case OpticalBombSight
				
			Case ImprovedOpticalBombSight
				
			Case AdvancedOpticalBombSight
				
			Case FireDirectionCenter
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(TargetingSystemMatrix)
			If TargetingSystemMatrix(i).ID = mvarDatatype Then
				If TargetingSystemMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		Select Case mvarDatatype
			Case LaserDesignator, LaserRangeFinder
				
				TempRange = System.Math.Round(mvarRange / 5, 0)
				
				'get base stats
				mvarWeight = TempRange * TargetingSystemMatrix(mvarMatrixPos).Weight
				mvarCost = TempRange * TargetingSystemMatrix(mvarMatrixPos).Cost
				mvarVolume = TempRange * TargetingSystemMatrix(mvarMatrixPos).Volume
			...
			Case Else
				'get base stats
				mvarWeight = TargetingSystemMatrix(mvarMatrixPos).Weight
				mvarCost = TargetingSystemMatrix(mvarMatrixPos).Cost
				mvarVolume = TargetingSystemMatrix(mvarMatrixPos).Volume
				mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
				'UPGRADE_WARNING: Couldn't resolve default property of object CalcComponentHitpoints(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				mvarHitPoints = CalcComponentHitpoints(RugHitMod * mvarSurfaceArea)
		...
		mvarPowerReqt = TargetingSystemMatrix(mvarMatrixPos).Power
		...
		If mvarRange <> 0 Then
			sPrint2 = ", " & VB6.Format(mvarRange, p_sFormat) & " mile range"
		Else
			sPrint2 = ""
		End If
