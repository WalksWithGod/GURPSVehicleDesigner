Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsVehicleAccess_NET.clsVehicleAccess")> Public Class clsVehicleAccess
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Or (InstallPoint = Leg) Or (InstallPoint = Arm) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Vehicle Access devices must be placed in Body, Superstructure, Turret, Popturret, Pod, equipment Pod, Leg or Arm.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			...
			Case ArmoredPassageTube
				
				'NOTE: Doors and hatches should be entered as 'Details' in the options dialog
				'Case Door
				'    mvarDescription = "Door"
				'    mvarCustomDescription = "Door"
				'Case Hatch
				'    mvarDescription = "Hatch"
				'    mvarCustomDescription = "Hatch"
			Case CargoRamp
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(VehicleAccessMatrix)
			If VehicleAccessMatrix(i).ID = mvarDatatype Then
				If VehicleAccessMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		Select Case mvarDatatype
			Case Airlock, MembraneAirlock
				mvarWeight = mvarRating * VehicleAccessMatrix(mvarMatrixPos).Weight
				mvarCost = mvarRating * VehicleAccessMatrix(mvarMatrixPos).Cost
				mvarVolume = mvarRating * VehicleAccessMatrix(mvarMatrixPos).Volume
				...
				mvarPowerReqt = mvarRating * VehicleAccessMatrix(mvarMatrixPos).Power
				
			Case Else
				
				mvarWeight = VehicleAccessMatrix(mvarMatrixPos).Weight
				mvarCost = VehicleAccessMatrix(mvarMatrixPos).Cost
				mvarVolume = VehicleAccessMatrix(mvarMatrixPos).Volume
				mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
				'UPGRADE_WARNING: Couldn't resolve default property of object CalcComponentHitpoints(). Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				mvarHitPoints = CalcComponentHitpoints(RugHitMod * mvarSurfaceArea)
				mvarPowerReqt = VehicleAccessMatrix(mvarMatrixPos).Power
		End Select
		...
		If mvarRating <> 0 Then
			sPrint1 = sPrint1 & VB6.Format(mvarRating) & " person "
		End If
