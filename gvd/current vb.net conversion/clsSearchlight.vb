Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsSearchlight_NET.clsSearchlight")> Public Class clsSearchlight
	...
	Public Property Range() As Single
		...
		Set(ByVal Value As Single)
			...
			Select Case mvarDatatype
				Case Headlight
					If mvarRange > gVehicleTL * 5 Then
						mvarRange = gVehicleTL * 5
						modHelper.InfoPrint(1, "Max range for headlights is TL * 5yards.")
					End If
					
				Case Searchlight, InfraredSearchlight
					If mvarRange > mvarTL Then
						modHelper.InfoPrint(1, "Max range for search lights is their Tech Level in miles.")
						mvarRange = mvarTL
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Searchlights must be placed in Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case Headlight
				mvarRange = gVehicleTL * 5 'in yards
				
			Case Searchlight
				mvarRange = 1 'in miles
			Case InfraredSearchlight
				mvarRange = 1 'in miles
		End Select
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(SearchlightMatrix)
			If SearchlightMatrix(i).ID = mvarDatatype Then
				If SearchlightMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'get base stats
		mvarWeight = SearchlightMatrix(mvarMatrixPos).Weight * Range
		mvarCost = mvarRange * SearchlightMatrix(mvarMatrixPos).Cost
		mvarVolume = mvarRange * SearchlightMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = SearchlightMatrix(mvarMatrixPos).Power * mvarRange
		...
		If mvarDatatype = Headlight Then
			sRange = " yard range"
		Else
			sRange = " mile range"
		End If
		
		mvarPrintOutput = NumericToString(mvarQuantity) & " TL" & mvarTL & " " & sPrint1 & mvarCustomDescription & sPrintPlural & ", " & VB6.Format(mvarRange, p_sFormat) & sRange & " (" & mvarLocation & sPrintDirection & ", HP " & mvarHitPoints & sPrintPlural2 & ", " & sPrintPlural3 & VB6.Format(mvarWeight, p_sFormat) & " lbs., " & VB6.Format(mvarVolume, p_sFormat) & " cf., " & "$" & VB6.Format(mvarCost, p_sFormat) & ", " & VB6.Format(mvarPowerReqt, p_sFormat) & " kW)." & mvarComment
