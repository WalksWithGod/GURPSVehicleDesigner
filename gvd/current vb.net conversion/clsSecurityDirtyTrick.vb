Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsSecurityDirtyTrick_NET.clsSecurityDirtyTrick")> Public Class clsSecurityDirtyTrick
	...
	Public Function LocationCheck() As Boolean
		...
		Select Case mvarDatatype
			...
			Case MutableLicensePlate
				If InstallPoint = Body Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "License Plate must be placed on hull.")
					TempCheck = False
				End If
			Case BrigsandRestraints
				If (InstallPoint = CrampedSeat) Or (InstallPoint = NormalSeat) Or (InstallPoint = RoomySeat) Or (InstallPoint = CycleSeat) Or (InstallPoint = Hammock) Or (InstallPoint = Bunk) Or (InstallPoint = Cabin) Or (InstallPoint = LuxuryCabin) Or (InstallPoint = Suite) Or (InstallPoint = LuxurySuite) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "Brigs and Restraints must be added to Seats or Accomodations such as Bunks, Cabins and Suites.")
					TempCheck = False
				End If
			Case Else
				If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Pod) Or (InstallPoint = Arm) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then
					TempCheck = True
				Else
					modHelper.InfoPrint(1, "This particular device must be placed in Body, Superstructure, Turret, Popturret, Pod, equipment Pod, Arm, Leg or Module.")
					TempCheck = False
				End If
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(SecurityMatrix)
			If SecurityMatrix(i).ID = mvarDatatype Then
				If SecurityMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		If mvarDatatype = BrigsandRestraints Then
			mvarQuantity = 1
			'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			mvarCost = Veh.Components(mvarParent).Cost * 0.1
		Else
			'get base stats
			mvarWeight = SecurityMatrix(mvarMatrixPos).Weight
			mvarCost = SecurityMatrix(mvarMatrixPos).Cost
			mvarVolume = SecurityMatrix(mvarMatrixPos).Volume
			...
			mvarPowerReqt = SecurityMatrix(mvarMatrixPos).Power
			'get finals
