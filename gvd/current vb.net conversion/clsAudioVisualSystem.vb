Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsAudioVisualSystem_NET.clsAudioVisualSystem")> Public Class clsAudioVisualSystem
	
	Private mvarLowlight As Boolean
	Private mvarInfrared As Boolean
	...
	Public Property Infrared() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.Infrared
			Infrared = mvarInfrared
		End Get
	...
	Public Property Lowlight() As Boolean
		Get
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.Lowlight
			Lowlight = mvarLowlight
		End Get
		Set(ByVal Value As Boolean)
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.Lowlight = 5
	...
	Public Function LocationCheck() As Boolean
		...
		ElseIf (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Superstructure) Or (InstallPoint = Pod) Or (InstallPoint = Turret) Or (InstallPoint = Popturret) Or (InstallPoint = Arm) Or (InstallPoint = Wing) Or (InstallPoint = OpenMount) Or (InstallPoint = Leg) Or (InstallPoint = equipmentPod) Or (InstallPoint = Module_Renamed) Then 
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Audiovisual Systems must be placed in Periscope, Body, Superstructure, Pod, equipment Pod, Turret, Popturret, Arm, Wing, Open Mount, Leg or Module.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			...
			Case DigitalReconCamera
				
			Case DigitalVehicleCamera
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(AVSystemMatrix)
			If AVSystemMatrix(i).ID = mvarDatatype Then
				If AVSystemMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		'lowlight option modifier
		If Lowlight Then
			LowlightMod = 2
		Else
			LowlightMod = 1
		End If
		'infrared option modifier
		If mvarInfrared Then
			LowlightMod = LowlightMod * 2
		End If
		
		' Calculate the Weight
		mvarWeight = AVSystemMatrix(mvarMatrixPos).Weight
		mvarCost = LowlightMod * AVSystemMatrix(mvarMatrixPos).Cost
		mvarVolume = AVSystemMatrix(mvarMatrixPos).Volume
		...
		mvarPowerReqt = AVSystemMatrix(mvarMatrixPos).Power
		
		'calc quantity and ruggedized finals
		...
		mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
		mvarPowerReqt = System.Math.Round(QRugMod * mvarPowerReqt, 2)
		...
		Else
			sPrint1 = ""
		End If
		If mvarLowlight Then
			sPrint2 = ", low light"
		End If
		If mvarInfrared Then
			If sPrint2 = "" Then
				sPrint2 = ", infrared option"
			Else
				sPrint2 = sPrint2 & " and infrared options"
			End If
		Else
			sPrint2 = sPrint2 & " option"
		End If
