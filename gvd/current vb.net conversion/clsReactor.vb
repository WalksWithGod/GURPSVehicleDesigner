Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsReactor_NET.clsReactor")> Public Class clsReactor
	...
	Public Property PowerConsumptionKeyChain() As Object
		Get
			Dim mvarPowerConsumptionKeyChain As Object
			'used when retrieving value of a property, on the right side of an assignment.
			'Syntax: Debug.Print X.PowerConsumptionKeyChain
		...
		Set(ByVal Value As Object)
			Dim mvarPowerConsumptionKeyChain As Object
			'used when assigning a value to the property, on the left side of an assignment.
			'Syntax: X.PowerConsumptionKeyChain = 5
	...
	Public Property Output() As Single
		...
		Set(ByVal Value As Single)
			...
			If (mvarOutput < 5) And (ReactorMatrix(mvarMatrixPos).MinPower = 5) Then
				mvarOutput = 5
				modHelper.InfoPrint(1, "Minimum power ouput for this type of reactor is 5kw")
			End If
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Or (InstallPoint = Wing) Or (InstallPoint = equipmentPod) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Reactors must go in Body, Pod, equipment Pod or Wing.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Dim mvarFuelType As Object
		Select Case mvarDatatype
			Case FissionReactor
				
				'UPGRADE_WARNING: Couldn't resolve default property of object mvarFuelType. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				mvarFuelType = UraniumFuelRod
			Case RTGReactor
			...
			Case TotalConversionPowerPlant
				
			Case CosmicPowerPlant
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(ReactorMatrix)
			If ReactorMatrix(i).ID = mvarDatatype Then
				If ReactorMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		If mvarDesiredOutput < 5 Then
			WeightMod = 0
			mvarWeight = ReactorMatrix(mvarMatrixPos).Weight1
			If mvarWeight <= 0 Then
				modHelper.InfoPrint(1, "Output must be 5kW or greater for this system.")
				mvarDesiredOutput = 5
				WeightMod = ReactorMatrix(mvarMatrixPos).Weight3
				mvarWeight = ReactorMatrix(mvarMatrixPos).Weight2
			End If
		Else
			WeightMod = ReactorMatrix(mvarMatrixPos).Weight3
			mvarWeight = ReactorMatrix(mvarMatrixPos).Weight2
		...
		mvarWeight = (mvarDesiredOutput * mvarWeight) + WeightMod
		
		'Find the volume
		mvarVolume = mvarWeight / ReactorMatrix(mvarMatrixPos).Volume
		
		'make sure minimum cost is met and that the CORE cost is addded
		mvarCost = (mvarWeight * ReactorMatrix(mvarMatrixPos).Cost) + ReactorMatrix(mvarMatrixPos).CoreCost
		...
		mvarCost = modPerformance.Maximum(mvarCost, ReactorMatrix(mvarMatrixPos).MinCost)
		
		'determine added fuel cost for Fission Reactor
		If mvarDatatype = FissionReactor Then
			TempFuel = mvarDesiredOutput / ReactorMatrix(mvarMatrixPos).FuelNeeded
			If TempFuel > System.Math.Round(TempFuel, 0) Then
				TempFuel = System.Math.Round(TempFuel, 0) + 1
			Else
				TempFuel = System.Math.Round(TempFuel, 0)
			End If
			mvarFuelConsumption = TempFuel
			mvarFuelCost = ReactorMatrix(mvarMatrixPos).FuelCost * TempFuel
		End If
		
		'get the endurance of the reactor
		mvarEndurance = ReactorMatrix(mvarMatrixPos).years
		
		'get rest of the base stats
		...
		mvarFuelCost = mvarFuelCost * mvarQuantity
		mvarCost = System.Math.Round((QRugMod * mvarCost) + mvarFuelCost, 2)
		...
		sPrint2 = ", lasts " & VB6.Format(mvarEndurance, p_sFormat) & " yrs" & sPrintPlural3
