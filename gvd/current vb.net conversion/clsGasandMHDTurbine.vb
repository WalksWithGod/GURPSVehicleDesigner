Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsGasandMHDTurbine_NET.clsGasandMHDTurbine")> Public Class clsGasandMHDTurbine
	...
	Public Function LocationCheck() As Boolean
		...
		If (InstallPoint = Body) Or (InstallPoint = GroupComponent) Or (InstallPoint = Pod) Or (InstallPoint = Wing) Then
			TempCheck = True
		Else
			modHelper.InfoPrint(1, "Gas and MHD Turbines must go in Body, Pod or Wing.")
			TempCheck = False
		End If
	...
	Public Sub Init()
		Select Case mvarDatatype
			Case OptimizedGasTurbine
				
				mvarFuelType = "gasoline"
			Case StandardGasTurbine
				
				mvarFuelType = "gasoline"
			Case HPGasTurbine
				
				mvarFuelType = "jet fuel"
			Case StandardMHDTurbine
				
				mvarFuelType = "hydrogen fuel"
			Case HPMHDTurbine
	...
	Public Sub GetMatrixIndex()
		...
		For i = 1 To UBound(TurbineMatrix)
			If TurbineMatrix(i).ID = mvarDatatype Then
				If TurbineMatrix(i).TL >= mvarTL Then
					mvarMatrixPos = i
					Exit For
				Else
					mvarMatrixPos = i
	...
	Public Sub StatsUpdate()
		...
		If mvarDesiredOutput < 5 Then
			WeightMod = 0
			TrueWeight = TurbineMatrix(mvarMatrixPos).Weight1
		Else
			WeightMod = TurbineMatrix(mvarMatrixPos).Weight3
			TrueWeight = TurbineMatrix(mvarMatrixPos).Weight2
		End If
		
		TrueWeight = (mvarDesiredOutput * TrueWeight) + WeightMod
		
		'Find the volume
		TempVolume = TrueWeight / TurbineMatrix(mvarMatrixPos).Volume
		
		
		'make sure minimum cost is met (this is done BEFORE closed cycle calcs)
		TempCost = TrueWeight * TurbineMatrix(mvarMatrixPos).Cost
		If TempCost < TurbineMatrix(mvarMatrixPos).MinCost Then TempCost = TurbineMatrix(mvarMatrixPos).MinCost
		
		'add weight modifier for closed cycle engine
		If mvarClosedCycle Then
			If (mvarDatatype = StandardMHDTurbine) Or (mvarDatatype = HPMHDTurbine) Then
				'nothing
			Else
				TrueWeight = 1.5 * TrueWeight
				TempCost = 1.5 * TempCost
				TempVolume = 1.5 * TempVolume
			End If
		End If
		...
		mvarSurfaceArea = CalcSurfaceArea(mvarVolume)
		mvarOutput = mvarQuantity * mvarDesiredOutput
		
		'determine fuel consumption
		TempFuel = TurbineMatrix(mvarMatrixPos).FuelUsed * mvarOutput
		'only alcohol fuel causes change in fuel consumption for turbines
		If mvarFuelType = "alcohol" Then
			mvarFuelConsumption = System.Math.Round(1.2 * TempFuel, 2)
		Else
			mvarFuelConsumption = System.Math.Round(TempFuel, 2)
		End If
		
		'calc liquid oxygen consumption for closed cycled operation.
		'this calc  must be done after the above normal fuel calcs
		If mvarClosedCycle Then
			If (mvarDatatype = StandardMHDTurbine) Or (mvarDatatype = HPMHDTurbine) Then
				mvarLOXConsumption = System.Math.Round(0.5 * mvarFuelConsumption, 2)
			Else
				mvarLOXConsumption = System.Math.Round(2.35 * mvarFuelConsumption, 2)
			End If
		End If
		...
		If mvarClosedCycle Then sPrint3 = ", closed cycle"
		...
		If mvarLOXConsumption <> 0 Then sPrint2 = sPrint2 & " and " & VB6.Format(mvarLOXConsumption, p_sFormat) & " gph liquid oxygen"
