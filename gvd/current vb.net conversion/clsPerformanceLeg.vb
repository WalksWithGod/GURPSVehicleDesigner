Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("clsPerformanceLeg_NET.clsPerformanceLeg")> Public Class clsPerformanceLeg
	...
	Public Property gTotalMotivePower() As Double
		Get
			'used when retrieving value of a property, on the right side of an assignment.
	...
	Public Sub CalcPerformance()
		...
		If mvarMotiveAssemblyKey = "" Then
			modHelper.InfoPrint(0, "No Leg Subassembly detected on vehicle.  Cannot caclulate Legged Ground performance.")
			Exit Sub
		Else
			...
			If Not TypeOf Veh.Components(mvarMotiveAssemblyKey) Is clsLeg Then
				modHelper.InfoPrint(0, "No Leg Subassemblies detected on vehicle.  Cannot caclulate Legged Ground performance.")
				Exit Sub
			End If
		End If
		...
		Call GetVehicleWeight(PERFORMANCELEG, mvarPercentAuxVehicleWeight, mvarPercentCargoWeight, mvarPercentAmmunitionWeight, mvarPercentHardpointWeight, mvarPercentFuelWeight, mvarPercentProvisionWeight, m_VWeight, m_VMass)
	...
	Function CalcGroundSpeed() As Single
		...
		For i = 1 To UBound(mvarKeyChain)
			...
			If CDbl(dType) = LegDrivetrain Then
				'UPGRADE_WARNING: Couldn't resolve default property of object Veh.Components. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		...
		TempSpeed = System.Math.Sqrt(mvargTotalMotivePower / m_VMass)
