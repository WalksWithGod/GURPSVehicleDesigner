...
Dim bVTStandIn As Boolean
    ...
    If KeyChain(1) = "" Then 'exit if there are no propulsion systems in the keychain
         Exit Function
    End If
...
ElseIf (VehicleHasWings) Or (VehicleHasRotors) Then
    ...
    If bResponsive Then
         TempTL = TempTL + 1
    End If
    If AllWingsAreHighAgility Then
         TempTL = TempTL + 1
    End If
    If AllWingsAreVariableSweep Then
         TempTL = TempTL + 1
    End If
    If VehicleHasCompControls Then
         TempTL = TempTL + 1
    End If
    If AllWingsRotorsControlledInstability Then
         TempTL = TempTL + 2
    End If
    If VehicleHasMMRRotors Then
         TempTL = TempTL - 1
    End If
...
If Veh.Components(BODY_KEY).LiftingBody Then
     TempSR = TempSR - 1
End If
If VehicleHasBipeorTripWings Then
     TempSR = TempSR - 1
End If
If AllWingsRotorsControlledInstability Then
     TempSR = TempSR - 1
End If
If (Veh.Surface.Stealth = "radical") And (VehicleHasWings) Then
     TempSR = TempSR - 1
End If
