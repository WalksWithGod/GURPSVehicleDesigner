...
Sub CalcWaterMRandSR(ByRef SR As Single, ByRef MR As Single, ByRef KeyChain As Variant, ByVal bResponsive As Boolean)
    ...
    If KeyChain(1) = "" Then 'dont perform routine if there are no propulsion systems in the keychain
         Exit Sub
    End If
    ...
    With Veh.Components(BODY_KEY)
        TempTech = .TL
        If TempTech > 11 Then '11 is the highest
             TempTech = 11
        End If
        If .Volume <= 100 Then
            Category = 1
    ...
    If Veh.Options.RollStabilizers Then
         SRBonus = SRBonus + 1
    End If
